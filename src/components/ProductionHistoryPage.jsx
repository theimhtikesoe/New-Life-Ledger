...
export default function ProductionHistoryPage() {
  ...
  const totalTubePieces = groups.reduce((sum, group) => sum + group.rows.filter((row) => row.category === "tube").reduce((rowSum, row) => rowSum + Number(row.outputQuantity || 0) * Number(row.outputCapacity || 0), 0), 0);
  const totalTubeDamageKg = groups.reduce((sum, group) => sum + Number(group.tubeMetrics?.tubeDamageKg || 0), 0);
  const totalScrapKg = groups.reduce((sum, group) => sum + Number(group.tubeMetrics?.scrapKg || 0), 0);
  const totalGlueWasteKg = groups.reduce((sum, group) => sum + Number(group.tubeMetrics?.glueWasteKg || 0), 0);
  ...
  return (
    <main className="mx-auto w-full max-w-5xl space-y-4 px-3 pb-8 sm:px-6">
      ...
      <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
        ...
        <div className="rounded-xl border border-orange-200 bg-orange-50 p-4"><p className="text-sm font-bold text-orange-700">Tube စုစုပေါင်း</p><p className="mt-1 text-2xl font-black text-orange-800">{formatNumber(totalTubePieces)} pcs</p></div>
        <div className="rounded-xl border border-red-200 bg-red-50 p-4"><p className="text-sm font-bold text-red-700">ဗူးပျက် / Tube ပျက်</p><p className="mt-1 text-lg font-black text-red-800">{formatNumber(totalWaste)} ဗူး · {formatNumber(totalTubeDamageKg)} kg</p></div>
        <div className="rounded-xl border border-violet-200 bg-violet-50 p-4"><p className="text-sm font-bold text-violet-700">ခုတ်ဖက် / ကော်ပျက်</p><p className="mt-1 text-lg font-black text-violet-800">{formatNumber(totalScrapKg)} kg · {formatNumber(totalGlueWasteKg)} kg</p></div>
        <div className="rounded-xl border border-slate-200 bg-slate-50 p-4"><p className="text-sm font-bold text-slate-700">မှတ်တမ်းအကြိမ်</p><p className="mt-1 text-2xl font-black text-slate-800">{formatNumber(groups.length)} ကြိမ်</p></div>
      </section>
