...
const CAP_CATALOG = [
  ...
  isCap: true,
  bottlesPerCard: 1,
  effectivePrice: { pricePerBottle: 40, source: "CAP_FIXED_PRICE" },
}));
...
function makeLine(item, cardCount) {
  ...
  const isCap = Boolean(item.isCap);
...
export default function SalesItemPicker({ catalog = [], saleItems = [], onChange, disabled = false }) {
  ...
  const pickerCatalog = useMemo(() => [...catalog, ...CAP_CATALOG], [catalog]);
  ...
  const previewBottles = selectedItem?.isCap ? previewCards : previewCards * Number(selectedItem?.bottlesPerCard || 0);
  const previewTotal = (selectedItem?.isCap ? previewCards : previewBottles) * selectedPrice;
  ...
  const totalCaps = saleItems.reduce((sum, item) => sum + (item.isCap ? Number(item.cardCount || item.unitCount || 0) : 0), 0);
  ...
  function addItem() {
    ...
    if (!Number(cardCount)) return setError(selectedItem.isCap ? "အဖုံးအရေအတွက် ထည့်ပါ။" : "ကဒ်အရေအတွက် ထည့်ပါ။");
  ...
  function updatePrice(id, value) {
    ...
    onChange(saleItems.map((item) => {
      if (item.id !== id) return item;
      const unitCount = item.isCap ? Number(item.cardCount || item.unitCount || 0) : Number(item.bottleCount || 0);
      ...
      return {
        ...
        pricePerCard: price * (item.isCap ? 1 : Number(item.capacity || 0)),
  ...
  return (
    <section className="space-y-3 rounded-xl border-2 border-violet-300 bg-violet-50/80 p-3 shadow-sm">
      <div className="flex items-start justify-between gap-3">
        ...
        <button type="button" onClick={openPicker} disabled={disabled || (!catalog.length && !CAP_CATALOG.length)} className="shrink-0 rounded-lg bg-violet-700 px-3 py-2 text-xs font-black text-white shadow-sm hover:bg-violet-800 disabled:cursor-not-allowed disabled:opacity-50">ထည့်ရန် +</button>
      </div>
      ...
      {saleItems.length ? <div className="space-y-3">{saleItems.map((item) => {
        ...
        return <div key={item.id} className="rounded-xl border-2 border-violet-200 bg-white p-3 shadow-sm">
          <div className="flex items-start justify-between gap-2"><div className="min-w-0"><p className="truncate text-sm font-black text-slate-900">{item.productName}{isCap ? "" : ` · ${item.capacity} ဆံ့`}</p><p className="mt-1 text-xs font-bold text-slate-600">{count.toLocaleString()} {isCap ? "အဖုံး" : "ဗူး"} · {formatMoney(item.pricePerBottle)}/{isCap ? "ဖုံး" : "ဗူး"} · {formatMoney(item.totalAmount)}</p></div><button type="button" onClick={() => onChange(saleItems.filter((line) => line.id !== item.id))} disabled={disabled} className="rounded-lg border border-rose-200 px-2 py-1 text-xs font-bold text-rose-600 hover:bg-rose-50">ဖျက်</button></div>
      ...
      {open ? <div className="space-y-3 rounded-xl border-2 border-violet-300 bg-white p-3 shadow-inner"><div className="flex items-center justify-between"><p className="text-sm font-black text-slate-900">ပစ္စည်းရွေးရန်</p><button type="button" onClick={() => setOpen(false)} className="rounded px-2 py-1 text-xs font-bold text-slate-500 hover:bg-slate-100">ပိတ်</button></div><select value={currentCategory} onChange={(event) => { setActiveCategory(event.target.value); setSelectedKey(""); }} disabled={disabled} className="h-10 w-full rounded-lg border-2 border-violet-200 bg-violet-50 px-3 text-sm font-bold text-slate-900">{categories.map((category) => <option key={category.key} value={category.key}>{category.label}</option>)}</select><select value={selectedKey} onChange={(event) => setSelectedKey(event.target.value)} disabled={disabled || !visibleItems.length} className="h-10 w-full rounded-lg border-2 border-violet-200 bg-white px-3 text-sm font-bold text-slate-900"><option value="">{currentCategory === "CAP" ? "အဖုံးအရောင် ရွေးပါ" : "ဗူးအမျိုးအစား ရွေးပါ"}</option>{visibleItems.map((item) => <option key={item.productKey} value={item.productKey}>{item.productName}{item.isCap ? ` · ${formatMoney(item.effectivePrice.pricePerBottle)}/ဖုံး` : ` · ${item.capacity} ဆံ့ · ${item.effectivePrice?.pricePerBottle ? `${formatMoney(item.effectivePrice.pricePerBottle)}/ဗူး` : "စျေးမသတ်မှတ်ရသေး"}`}</option>)}</select><div className="grid grid-cols-2 gap-3"><label className="text-xs font-bold text-slate-700">{selectedItem?.isCap ? "အဖုံးအရေအတွက်" : "ကဒ်အရေအတွက်"}<input type="number" min="1" step="1" value={cardCount} onChange={(event) => setCardCount(event.target.value)} disabled={disabled} className="mt-1 h-10 w-full rounded-lg border-2 border-violet-200 px-3 text-center text-base font-black text-slate-900" /></label><div className="rounded-lg border border-violet-200 bg-violet-50 px-3 py-2 text-xs text-violet-950"><p className="font-bold">အလိုအလျောက်တွက်ချက်မှု</p><p className="mt-1">{selectedItem ? (selectedItem.isCap ? `${previewCards.toLocaleString()} အဖုံး × 40 Ks` : `${previewCards} ကဒ် × ${Number(selectedItem.bottlesPerCard || 0)} = ${previewBottles.toLocaleString()} ဗူး`) : "ပစ္စည်းရွေးပါ"}</p><p className="mt-1 text-xl font-black">{formatMoney(previewTotal)}</p></div></div>{error ? <p role="alert" className="rounded-lg bg-rose-50 px-3 py-2 text-xs font-bold text-rose-700">{error}</p> : null}<button type="button" onClick={addItem} disabled={disabled || !selectedItem} className="h-10 w-full rounded-lg bg-violet-700 text-sm font-black text-white hover:bg-violet-800 disabled:opacity-50">ထည့်မည်</button></div> : null}
