...
export default function SalesItemPicker({ catalog = [], saleItems = [], onChange, disabled = false }) {
  ...
  const pickerCatalog = useMemo(() => catalog, [catalog]);
  ...
  return (
    <section className="space-y-3 rounded-xl border-2 border-violet-300 bg-violet-50/80 p-3 shadow-sm">
      <div className="flex items-start justify-between gap-3">
        ...
        <button type="button" onClick={openPicker} disabled={disabled || !catalog.length} className="shrink-0 rounded-lg bg-violet-700 px-3 py-2 text-xs font-black text-white shadow-sm hover:bg-violet-800 disabled:cursor-not-allowed disabled:opacity-50">ထည့်ရန် +</button>
      </div>
