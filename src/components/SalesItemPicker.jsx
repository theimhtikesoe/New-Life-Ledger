...
export default function SalesItemPicker({ catalog = [], saleItems = [], onChange, disabled = false }) {
  ...
  return (
    <section className="space-y-3 rounded-xl border-2 border-violet-300 bg-violet-50/80 p-3 shadow-sm">
      ...
      {saleItems.length ? <div className="space-y-3">{saleItems.map((item) => <div key={item.id} className="rounded-xl border-2 border-violet-200 bg-white p-3 shadow-sm"><div className="flex items-start justify-between gap-2"><div className="min-w-0"><p className="truncate text-sm font-black text-slate-900">{item.productName} · {item.capacity} ဆံ့</p><p className="mt-1 text-xs font-bold text-slate-600">{Number(item.bottleCount || 0).toLocaleString()} ဗူး · {formatMoney(item.pricePerBottle)}/ဗူး · {formatMoney(item.totalAmount)}</p></div><button type="button" onClick={() => onChange(saleItems.filter((line) => line.id !== item.id))} disabled={disabled} className="rounded-lg border border-rose-200 px-2 py-1 text-xs font-bold text-rose-600 hover:bg-rose-50">ဖျက်</button></div><div className="mt-3 grid grid-cols-2 gap-3 border-t border-violet-100 pt-3"><label className="text-xs font-bold text-slate-600">ကဒ်<input type="number" min="1" step="1" value={item.cardCount} onChange={(event) => updateCards(item.id, event.target.value)} disabled={disabled} className="mt-1 h-10 w-full rounded-lg border-2 border-violet-200 px-2 text-center text-base font-black text-slate-900" /></label><label className="text-xs font-bold text-slate-600">Customer စျေး/ဗူး<input type="number" min="0" step="1" value={item.customerPricePerBottle ?? item.pricePerBottle} onChange={(event) => updatePrice(item.id, event.target.value)} disabled={disabled} className="mt-1 h-10 w-full rounded-lg border-2 border-violet-200 px-2 text-right text-base font-black text-slate-900" /></label></div><p className="mt-2 border-t border-violet-100 pt-2 text-xs font-bold text-slate-500">{item.cardCount} ကဒ် × {item.capacity} = {Number(item.bottleCount || 0).toLocaleString()} ဗူး</p></div>)}</div> : <p className="rounded-lg border-2 border-dashed border-violet-300 bg-white/70 px-3 py-2 text-xs text-violet-800">ဗူးမရွေးရသေးပါ။ `ဗူးထည့်ရန် +` ကိုနှိပ်ပါ။</p>}

      {open ? <div className="space-y-3 rounded-xl border-2 border-violet-300 bg-white p-3 shadow-inner"><div className="flex items-center justify-between"><p className="text-sm font-black text-slate-900">ဗူးရွေးရန်</p><button type="button" onClick={() => setOpen(false)} className="rounded px-2 py-1 text-xs font-bold text-slate-500 hover:bg-slate-100">ပိတ်</button></div><select value={currentCategory} onChange={(event) => { setActiveCategory(event.target.value); setSelectedKey(""); }} disabled={disabled} className="h-10 w-full rounded-lg border-2 border-violet-200 bg-violet-50 px-3 text-sm font-bold text-slate-900">{categories.map((category) => <option key={category.key} value={category.key}>{category.label}</option>)}</select><select value={selectedKey} onChange={(event) => setSelectedKey(event.target.value)} disabled={disabled || !visibleItems.length} className="h-10 w-full rounded-lg border-2 border-violet-200 bg-white px-3 text-sm font-bold text-slate-900"><option value="">ဗူးအမျိုးအစား ရွေးပါ</option>{visibleItems.map((item) => <option key={item.productKey} value={item.productKey}>{item.productName} · {item.capacity} ဆံ့ {item.effectivePrice?.pricePerBottle ? `· ${formatMoney(item.effectivePrice.pricePerBottle)}/ဗူး` : "· စျေးမသတ်မှတ်ရသေး"}</option>)}</select><div className="grid grid-cols-2 gap-3"><label className="text-xs font-bold text-slate-700">ကဒ်အရေအတွက်<input type="number" min="1" step="1" value={cardCount} onChange={(event) => setCardCount(event.target.value)} disabled={disabled} className="mt-1 h-10 w-full rounded-lg border-2 border-violet-200 px-3 text-center text-base font-black text-slate-900" /></label><div className="rounded-lg border border-violet-200 bg-violet-50 px-3 py-2 text-xs text-violet-950"><p className="font-bold">အလိုအလျောက်တွက်ချက်မှု</p><p className="mt-1">{selectedItem ? `${previewCards} ကဒ် × ${Number(selectedItem.bottlesPerCard || 0)} = ${previewBottles.toLocaleString()} ဗူး` : "ဗူးအမျိုးအစား ရွေးပါ"}</p><p className="mt-1 text-xl font-black">{formatMoney(previewTotal)}</p></div></div>{error ? <p role="alert" className="rounded-lg bg-rose-50 px-3 py-2 text-xs font-bold text-rose-700">{error}</p> : null}<button type="button" onClick={addItem} disabled={disabled || !selectedItem} className="h-10 w-full rounded-lg bg-violet-700 text-sm font-black text-white hover:bg-violet-800 disabled:opacity-50">ဒီဗူးကို ထည့်မည်</button></div> : null}
