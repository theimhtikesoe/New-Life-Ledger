    ...
    <section className="space-y-3 rounded-xl border-2 border-violet-300 bg-violet-50/80 p-3 shadow-sm">
      ...
      {saleItems.length ? <div className="space-y-3">{saleItems.map((item) => {
        ...
        return <div key={item.id} className="rounded-xl border-2 border-violet-200 bg-white p-3 shadow-sm">
          ...
          {!isCap && !isTubeItem(item) && capItems.length ? <div className="mt-3 grid grid-cols-2 gap-3 border-t border-violet-100 pt-3"><label className="text-xs font-bold text-slate-600">နေရာ<select value={item.capLocation || "မန္တလေး"} onChange={(event) => updateCapConfig(item.id, "capLocation", event.target.value)} disabled={disabled} className="mt-1 h-10 w-full rounded-lg border-2 border-violet-200 bg-white px-2 text-sm font-black text-slate-900">{capLocations.map((location) => <option key={location}>{location}</option>)}</select></label><label className="text-xs font-bold text-slate-600">အဖုံးအရောင် · {item.capLocation || "မန္တလေး"}<select value={item.capProductKey || ""} onChange={(event) => updateCapConfig(item.id, "capProductKey", event.target.value)} disabled={disabled} className="mt-1 h-10 w-full rounded-lg border-2 border-violet-200 bg-white px-2 text-sm font-black text-slate-900"><option value="">အဖုံး မသတ်မှတ်ရသေး</option>{capItems.map((cap) => <option key={cap.productKey} value={cap.productKey}>{item.capLocation || "မန္တလေး"} · {cap.productName}</option>)}</select></label><label className="text-xs font-bold text-slate-600">အဖုံးပေးပုံ<select value={item.capDeliveryMode || "AUTO"} onChange={(event) => updateCapConfig(item.id, "capDeliveryMode", event.target.value)} disabled={disabled} className="mt-1 h-10 w-full rounded-lg border-2 border-violet-200 bg-white px-2 text-sm font-black text-slate-900"><option value="AUTO">ဗူးအတိုင်း Auto</option><option value="PARTIAL">အမှန်ပေးသည့်အတိုင်း</option></select></label>{item.capDeliveryMode === "PARTIAL" ? <label className="text-xs font-bold text-slate-600">အမှန်ပေးအဖုံး<input type="number" min="0" step="1" value={item.capActualCount ?? ""} onChange={(event) => updateCapConfig(item.id, "capActualCount", event.target.value)} disabled={disabled} className="mt-1 h-10 w-full rounded-lg border-2 border-violet-200 px-2 text-center text-base font-black text-slate-900" /></label> : null}<label className="text-xs font-bold text-slate-600">တစ်အိတ်ဆံ့<select value={item.capPackSize || 5000} onChange={(event) => updateCapConfig(item.id, "capPackSize", event.target.value)} disabled={disabled} className="mt-1 h-10 w-full rounded-lg border-2 border-violet-200 bg-white px-2 text-sm font-black text-slate-900"><option value="5000">5000</option><option value="100">100</option></select></label><label className="text-xs font-bold text-slate-600">အဖုံးအပို<input type="number" min="0" step="1" value={item.capExtraCount || ""} onChange={(event) => updateCapConfig(item.id, "capExtraCount", event.target.value)} disabled={disabled} className="mt-1 h-10 w-full rounded-lg border-2 border-violet-200 px-2 text-center text-base font-black text-slate-900" /><span className="mt-1 block text-[11px] text-slate-500">ပုံမှန် {Number(item.capNormalCount || item.bottleCount || 0).toLocaleString()} + အပို = {Number(item.capTotalCount || item.bottleCount || 0).toLocaleString()} ဖုံး</span></label></div> : null}
        </div>;
      })}</div> : null}
