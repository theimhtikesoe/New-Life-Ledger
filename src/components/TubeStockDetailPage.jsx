    ...
    <main className="app-page-main">
      <div className="app-page-container app-page-surface space-y-4 pt-4 sm:pt-5">
        ...
        {rows.length ? <section className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
          ...
          <div className="hidden overflow-x-auto sm:block"><table className="w-full min-w-[980px] text-sm"><thead className="bg-slate-50 text-left text-[11px] font-black text-slate-500"><tr><th className="px-3 py-2.5">Tube အမျိုးအစား</th><th className="px-3 py-2.5 text-right">ဆံ့</th><th className="px-3 py-2.5 text-right">ထုတ်လုပ်ဝင်</th><th className="px-3 py-2.5 text-right">သုံးစွဲ</th><th className="px-3 py-2.5 text-right">နဂို Tube သုံးပြီး</th><th className="px-3 py-2.5 text-right">လက်ကျန်</th><th className="px-3 py-2.5">အသေးစိတ်</th></tr></thead><tbody>{rows.map((row) => <tr key={`${row.tubeType}-${row.capacity}`} className="border-t border-slate-100"><td className="px-3 py-3 font-black text-slate-900">{row.tubeType}</td><td className="px-3 py-3 text-right text-xs font-bold text-slate-500">{number(row.capacity)} pcs/အိတ်</td><td className="px-3 py-3 text-right font-black text-emerald-700"><span className="text-base">{number(row.productionPacks)} အိတ်</span><br /><span className="text-xs">{number(row.productionPieces)} pcs</span></td><td className="px-3 py-3 text-right font-black text-rose-700"><span className="text-base">{number(row.usedPacks)} အိတ်</span><br /><span className="text-xs">{number(row.usedPieces)} pcs</span></td><td className="px-3 py-3 text-right font-black text-amber-700"><span className="text-base">{number(row.unrecordedOpeningPacks)} အိတ်</span><br /><span className="text-xs">{number(row.unrecordedOpeningPieces)} pcs</span></td><td className="px-3 py-3 text-right font-black text-indigo-700"><span className="text-base">{number(row.currentPacks)} အိတ်</span><br /><span className="text-xs">{number(row.currentPieces)} pcs</span></td><td className="px-3 py-3"><button type="button" onClick={() => openTypeDetails(row)} className="rounded-lg border border-blue-200 bg-blue-50 px-2.5 py-1.5 text-xs font-black text-blue-800">ကြည့်</button></td></tr>)}</tbody></table></div>
          <div className="divide-y divide-slate-100 sm:hidden">{rows.map((row) => <article key={`${row.tubeType}-${row.capacity}`} className="space-y-2.5 p-3"><div className="flex items-start justify-between gap-2"><div><h3 className="text-sm font-black text-slate-900">{row.tubeType}</h3><p className="text-[11px] font-bold text-slate-500">{number(row.capacity)} pcs/အိတ်</p></div><span className="text-base font-black text-indigo-700">{number(row.currentPacks)} အိတ်</span></div><div className="grid grid-cols-2 gap-1.5 text-center"><div className="rounded-lg bg-emerald-50 p-2"><p className="text-[10px] font-bold text-emerald-700">ဝင်</p><p className="text-sm font-black text-emerald-800">{number(row.productionPacks)} အိတ်</p><p className="text-[10px] font-bold text-emerald-700">{number(row.productionPieces)} pcs</p></div><div className="rounded-lg bg-rose-50 p-2"><p className="text-[10px] font-bold text-rose-700">သုံး</p><p className="text-sm font-black text-rose-800">{number(row.usedPacks)} အိတ်</p><p className="text-[10px] font-bold text-rose-700">{number(row.usedPieces)} pcs</p></div><div className="rounded-lg bg-amber-50 p-2"><p className="text-[10px] font-bold text-amber-700">နဂို Tube သုံးပြီး</p><p className="text-sm font-black text-amber-800">{number(row.unrecordedOpeningPacks)} အိတ်</p><p className="text-[10px] font-bold text-amber-700">{number(row.unrecordedOpeningPieces)} pcs</p></div><div className="rounded-lg bg-indigo-50 p-2"><p className="text-[10px] font-bold text-indigo-700">လက်ကျန်</p><p className="text-sm font-black text-indigo-800">{number(row.currentPacks)} အိတ်</p><p className="text-[10px] font-bold text-indigo-700">{number(row.currentPieces)} pcs</p></div></div><button type="button" onClick={() => openTypeDetails(row)} className="min-h-9 w-full rounded-lg border border-blue-200 bg-blue-50 px-2.5 py-1.5 text-xs font-black text-blue-800">ရက်စွဲအလိုက် အသေးစိတ်ကြည့်ရန်</button></article>)}</div>
        </section> : null}
