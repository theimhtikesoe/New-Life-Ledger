    ...
    <main className="app-page-main">
      <div className="app-page-container app-page-surface space-y-4 pt-5 sm:pt-6">
        ...
        {!loading && rows.length ? <>
          ...
          <section className="rounded-2xl border border-amber-200 bg-amber-50/60 p-4 shadow-sm"><h2 className="text-lg font-black text-amber-950">အလေးချိန်ဖြင့် သီးခြားမှတ်တမ်းများ</h2><p className="mt-1 text-sm font-bold text-amber-800">ကော်စေ့၊ ခုတ်ဖက်၊ Tube ပျက်နှင့် ကော်ပျက်တို့ကို Tube pcs အဖြစ် မရေတွက်ဘဲ သီးခြားသယ်ဆောင်/နောက်ပိုင်းလက်ကျန်တွက်ရန် စုစည်းထားပါသည်။</p><div className="mt-3 grid gap-2 text-sm sm:grid-cols-2 lg:grid-cols-4"><p className="rounded-xl bg-white p-3 font-black text-amber-900">သုံးကော်စေ့<br />{formatDecimal(materialTotals.usedGlueKg)} kg / {formatDecimal(materialTotals.usedGlueBags)} အိတ်</p><p className="rounded-xl bg-white p-3 font-black text-sky-900">ကျန်ကော်စေ့<br />{formatDecimal(materialTotals.remainingGlueKg)} kg / {formatDecimal(materialTotals.remainingGlueBags)} အိတ်</p><p className="rounded-xl bg-white p-3 font-black text-orange-900">ခုတ်ဖက်<br />{formatDecimal(materialTotals.scrapKg)} kg</p><p className="rounded-xl bg-white p-3 font-black text-rose-900">Tube ပျက်<br />{formatDecimal(materialTotals.tubeDamageKg)} kg</p><p className="rounded-xl bg-white p-3 font-black text-red-900">ကော်ပျက်<br />{formatDecimal(materialTotals.glueWasteKg)} kg</p><p className="rounded-xl bg-white p-3 font-black text-slate-900">ခုတ်ဖက် / ကော်စေ့<br />{formatDecimal(materialTotals.scrapTubeCount)} / {formatDecimal(materialTotals.scrapGlueCount)}</p></div></section>
        </> : null}
        ...
        {!loading && rows.length ? <section className="space-y-3">{rows.map((row) => { const metrics = row.tubeMetrics || {}; const workers = Array.isArray(row.involvedWorkers) ? row.involvedWorkers : []; return <article key={row.id} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><div className="flex flex-wrap items-start justify-between gap-2"><div><h2 className="text-lg font-black text-slate-900">{row.tubeG} {row.tubeColor}</h2><p className="text-sm font-bold text-slate-500">{row.machineName || row.machineCode} · {row.actorName || "User"}</p></div><span className="rounded-full bg-cyan-100 px-3 py-1 text-sm font-black text-cyan-800">{formatNumber(row.outputQuantity)} အိတ် × {formatNumber(row.outputCapacity)} pcs</span></div><div className="mt-2 rounded-xl border border-blue-200 bg-blue-50 p-2 text-sm font-bold text-blue-900">ပူးတွဲဆင်းသူများ: {workers.length ? workers.join(" · ") : "မရှိ"}</div><p className="mt-3 rounded-xl bg-cyan-50 p-3 text-base font-black text-cyan-950">စုစုပေါင်း {formatNumber(Number(row.outputQuantity || 0) * Number(row.outputCapacity || 0))} pcs</p><div className="mt-3 grid gap-2 text-sm sm:grid-cols-2 lg:grid-cols-3"><p className="rounded-lg bg-amber-50 p-2 font-bold text-amber-900">သုံးကော်စေ့: {formatDecimal(metrics.usedGlueKg)} kg / {formatDecimal(metrics.usedGlueBags)} အိတ်</p><p className="rounded-lg bg-sky-50 p-2 font-bold text-sky-900">ကျန်ကော်စေ့: {formatDecimal(metrics.remainingGlueBags)} အိတ် / {formatDecimal(metrics.remainingGlueKg)} kg</p><p className="rounded-lg bg-orange-50 p-2 font-bold text-orange-900">ခုတ်ဖက်: {formatDecimal(metrics.scrapKg)} kg</p><p className="rounded-lg bg-rose-50 p-2 font-bold text-rose-900">Tube ပျက်: {formatDecimal(metrics.tubeDamageKg)} kg</p><p className="rounded-lg bg-red-50 p-2 font-bold text-red-900">ကော်ပျက်: {formatDecimal(metrics.glueWasteKg)} kg</p><p className="rounded-lg bg-slate-100 p-2 font-bold text-slate-800">ခုတ်ဖက်/ကော်စေ့: {formatDecimal(metrics.scrapTubeCount)} / {formatDecimal(metrics.scrapGlueCount)}</p></div></article>; })}</section> : null}
      </div>
    </main>
  );
}
