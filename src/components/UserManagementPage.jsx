          ...
          <form onSubmit={savePermissions} className="grid gap-5 lg:grid-cols-[250px_minmax(0,1fr)]">
            ...
            <section className="rounded-2xl border border-violet-200 bg-white p-4 shadow-sm sm:p-5">
              ...
              <div className="mt-4 grid gap-2 sm:grid-cols-[minmax(0,1fr)_220px]">
                ...
                <select value={category} onChange={(event) => setCategory(event.target.value)} className="min-h-12 rounded-xl border border-slate-300 bg-white px-3 py-2.5 text-sm font-bold leading-6 text-slate-800 outline-none focus:border-cyan-500 focus:ring-2 focus:ring-cyan-100">{categories.map((item) => <option key={item} value={item}>{item === "ALL" ? `အားလုံး (${PERMISSION_PAGES.length})` : item}</option>)}</select>
              </div>
