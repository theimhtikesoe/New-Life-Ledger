...
import { BOTTLE_GROUPS, BOTTLE_ITEMS, DEFAULT_TUBE_MAPPINGS, TUBE_BY_MACHINE, getBottleDisplayName, getBottleGroup, getBottleUnit, MACHINES, normalizeTubeTypes } from "@/lib/production-catalog";
...
export default function ProductionEntryPage() {
  ...
  const [tubeSelections, setTubeSelections] = useState({});
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (draft && typeof draft === "object") {
        ...
        if (draft.tubeSelections && typeof draft.tubeSelections === "object") setTubeSelections(draft.tubeSelections);
  ...
  useEffect(() => {
    if (category === "tube" && tubeItems.length && !tubeItems.some((item) => `${item.g}|${item.color}|${item.pcsPerBag}` === activeTubeKey)) {
      setActiveTubeKey(`${tubeItems[0].g}|${tubeItems[0].color}|${tubeItems[0].pcsPerBag}`);
    }
  ...
  }, [actorName, reportDate, historyDate, machineCode, category, activeBottleGroup, activeTubeKey, lines, tubeSelections, wasteQuantity, tubeDamageQuantity, tubeQuantity, tubeQuantityUnit, tubeMetrics, involvedWorkers, workerNameDraft, editingSubmissionId]);
  ...
  const entries = useMemo(() => {
    if (category === "tube") {
      return tubeItems.map((item) => ({
        key: `${item.g}|${item.color}|${item.pcsPerBag}`,
        label: item.label,
    ...
    return BOTTLE_ITEMS.flatMap((item) => item.capacities.map((capacity) => ({
      ...
      tubeOptions: normalizeTubeTypes(DEFAULT_TUBE_MAPPINGS[`${item.type}::${capacity}`]),
    })));
  }, [category, tubeItems]);
  ...
  const filledEntries = useMemo(() => entries
    .map((entry) => {
      ...
      return { ...entry, quantity, outputQuantity: quantity, outputCapacity: entry.capacity, tubeType: category === "bottle" ? (tubeSelections[entry.key] || entry.tubeOptions?.[0] || null) : null };
    })
    .filter((entry) => entry.quantity > 0), [activeTubeKey, category, entries, lines, tubeMetrics.tubeCountBags, tubeSelections]);
  ...
  const summaryByType = useMemo(() => {
    ...
    filledEntries.forEach((entry) => {
      const key = category === "tube"
        ? `${entry.tubeG} ${entry.tubeColor} · ${entry.capacity.toLocaleString()} ဆံ့`
  ...
  const groupedHistory = useMemo(() => {
    ...
    filteredHistory.forEach((row) => {
      ...
      group.tubeMetrics = { ...(group.tubeMetrics || {}), ...(row.tubeMetrics && typeof row.tubeMetrics === "object" ? row.tubeMetrics : {}) };
      group.rows.push(row);
  ...
  function startEdit(group) {
    ...
    setTubeSelections(Object.fromEntries(group.rows.map((row) => [entryKeyForRow(row), row.tubeType || ""])));
  ...
  async function submit(event) {
    ...
    const typeLines = summaryByType.map((item) => category === "tube"
      ? `${item.label}: ${formatNumber(item.cards)} အိတ် / ${formatNumber(item.pieces)} pcs`
      : `${item.label}: ${formatNumber(item.cards)} ${item.unit} / ${formatNumber(item.pieces)} ဗူး`).join("\n");
    const selectedTube = tubeItems.find((item) => `${item.g}|${item.color}|${item.pcsPerBag}` === activeTubeKey);
    const persistedTubeMetrics = category === "tube"
      ? { ...tubeMetrics, tubeCountPcs: Number(tubeMetrics.tubeCountBags || 0) * Number(selectedTube?.pcsPerBag || tubeMetrics.tubeCountPcs || 0) }
      : null;
    ...
    <main className="app-page-main production-page">
      <div className="app-page-container app-page-surface production-container">
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <div className="grid grid-cols-1 items-start gap-4 md:grid-cols-3">
            <div className="relative z-30 flex min-w-0 flex-col gap-2 overflow-visible rounded-2xl border-2 border-violet-200 bg-violet-50 p-4 text-base font-black text-violet-950 shadow-sm"><span className="text-lg">အမျိုးအစား</span><ThemedSelect ariaLabel="အမျိုးအစား" value={category} onChange={(event) => handleCategoryChange(event.target.value)} className="h-14 w-full rounded-xl border-2 border-violet-300 bg-white px-3 text-lg font-black"><option value="bottle">ဗူးခွံ</option><option value="tube">Tube</option></ThemedSelect></div>
            <div className="flex min-w-0 flex-col gap-2 rounded-2xl border-2 border-orange-200 bg-orange-50 p-4 text-base font-black text-orange-950 shadow-sm"><RequiredLabel>စက်</RequiredLabel><ThemedSelect ariaLabel="စက်" value={machineCode} onChange={(event) => setMachineCode(event.target.value)} className="h-14 w-full rounded-xl border-2 border-orange-300 bg-white px-3 text-lg font-black"><option value="">စက်ရွေးချယ်ရန်</option>{MACHINES.filter((machine) => !machine.category || machine.category === category).map((machine) => <option key={machine.code} value={machine.code}>{machine.name}</option>)}</ThemedSelect></div>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
        <div className="mb-4 flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
          <div><h2 className="text-2xl font-black text-slate-800">{category === "tube" ? "Tube ထွက်ရှိမှုမှတ်တမ်းများ" : "ဗူး ထွက်ရှိမှုမှတ်တမ်းများ"}</h2><p className="text-xs text-slate-500">Date Filter ဖြင့် မနေ့၊ ဒီနေ့နှင့် ရှေ့နေ့များ၏ record များကို ကြည့်နိုင်ပါသည်။</p></div>
