  ...
  return (
    <main className="app-page-main production-page">
      <div className="app-page-container app-page-surface production-container">
        ...
        <section className="grid gap-5 lg:grid-cols-2"><div className="rounded-2xl border border-red-200 bg-white p-4 shadow-sm"><h2 className="mb-3 text-lg font-black text-red-800">ပျက်စီးမှု <span className="font-normal text-slate-400">(မဖြည့်လည်းရ)</span></h2><label className="text-sm font-bold text-slate-700"><OptionalLabel>ပျက်စီးသွားသော ဗူးရေအတိအကျ</OptionalLabel><input type="number" min="0" step="1" value={wasteQuantity} onFocus={() => { if (wasteQuantity === "0") setWasteQuantity(""); }} onChange={(event) => setWasteQuantity(event.target.value === "" ? "" : String(Number(event.target.value)))} className="mt-1 w-full rounded-xl border border-slate-300 p-3 font-bold" /></label><label className="mt-3 block text-sm font-bold text-slate-700"><span>အကြောင်းရင်း {damagedPieces > 0 ? <span className="text-red-600">*</span> : <span className="font-normal text-slate-400">(ပျက်စီးမှုရှိမှ ဖြည့်ရန်)</span>}</span><input value={wasteNote} onChange={(event) => setWasteNote(event.target.value)} placeholder={damagedPieces > 0 ? "ပျက်စီးရခြင်းအကြောင်းရင်း ထည့်ပါ" : "ပျက်စီးမှုမရှိလျှင် မဖြည့်လည်းရ"} className="mt-1 w-full rounded-xl border border-slate-300 p-3" /></label></div>
