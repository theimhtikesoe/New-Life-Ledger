    ...
    <main className="app-page-main production-page">
      <div className="app-page-container app-page-surface production-container">
        ...
        <section className="rounded-2xl border border-violet-200 bg-white p-4 shadow-sm">
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{visibleEntries.map((entry) => { const isBlue = entryIsBlue(entry); return <label key={entry.key} className={`min-w-0 rounded-2xl border-2 p-4 text-base font-bold shadow-sm ${isBlue ? "border-emerald-300 bg-emerald-50 text-slate-900" : "border-slate-300 bg-white text-slate-900"}`}><span className="flex min-w-0 items-start justify-between gap-2"><span className="min-w-0 flex-1 break-words whitespace-normal text-lg font-black leading-snug text-slate-900">{entry.label}</span><span className={`shrink-0 rounded-full px-2.5 py-1 text-sm font-black ${isBlue ? "border border-emerald-300 bg-emerald-100 text-emerald-900" : "border border-slate-400 bg-slate-50 text-slate-700"}`}>{entry.capacity.toLocaleString()} {category === "tube" ? "ဗူး/အိတ်" : "ဆံ့"}</span></span><span className="mt-2 flex min-w-0 items-start gap-2 break-words whitespace-normal text-sm font-bold text-slate-700"><span className={`shrink-0 rounded-full border px-2 py-0.5 ${isBlue ? "border-emerald-300 bg-emerald-100 text-emerald-900" : "border-slate-400 bg-slate-100 text-slate-700"}`}>{isBlue ? "ပြာ" : "ဖြူ"}</span><span className="min-w-0 break-words">{category === "tube" ? entry.sub : `${entry.unit} အရေအတွက် — ${entry.capacity.toLocaleString()} ဆံ့`}</span></span><input type="number" min="0" step="1" value={lines[entry.key] || ""} onFocus={() => { clearZeroOnFocus(entry.key); focusField(`line:${entry.key}`); }} onBlur={blurField} onChange={(event) => updateLine(entry.key, event.target.value)} placeholder="0" className={`mt-2 w-full rounded-xl border-2 bg-white p-4 text-right text-2xl font-black outline-none ${isBlue ? "border-sky-300 focus:border-sky-500" : "border-slate-300 focus:border-slate-600"} ${focusedField === `line:${entry.key}` ? "ring-4 ring-amber-300 shadow-lg" : ""}`} /></label>; })}</div>
