  ...
  return (
    <main className="app-page-main production-page">
      <div className="app-page-container app-page-surface production-container">
        ...
        {category === "bottle" && visibleEntries.some((entry) => entry.tubeOptions?.length) ? <section className="mb-4 rounded-2xl border border-cyan-200 bg-cyan-50 p-4 shadow-sm"><h3 className="text-base font-black text-cyan-950">ချိတ်ဆက်မည့် Tube</h3><p className="mt-1 text-xs text-cyan-800">0.9 ပြာနှင့် 1 လီတာ ပြာတို့တွင် သက်ဆိုင်ရာ ဆံ့အလိုက် S+1 နှင့် S+S Tube နှစ်မျိုးလုံးကို အသုံးပြုနိုင်ပါသည်။ ရွေးချယ်ရန်မလိုပါ။</p><div className="mt-3 grid gap-2 sm:grid-cols-2">{visibleEntries.filter((entry) => entry.tubeOptions?.length).map((entry) => <div key={`tube-map-${entry.key}`} className="rounded-xl border border-cyan-200 bg-white p-3 text-sm font-bold"><span className="block text-cyan-950">{entry.label} · {entry.capacity} ဆံ့</span><div className="mt-2 flex flex-wrap gap-2">{entry.tubeOptions.map((tubeType) => <span key={tubeType} className="rounded-lg border border-cyan-300 bg-cyan-50 px-3 py-2 font-black text-cyan-950">{tubeType}</span>)}</div></div>)}</div></section> : null}
