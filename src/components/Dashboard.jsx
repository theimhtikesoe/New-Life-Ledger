      ...
      <div className="dashboard-content-stack mx-auto flex w-full min-w-0 max-w-7xl flex-col gap-6 overflow-x-clip px-3 py-3 sm:gap-6 sm:px-6 sm:py-6 lg:px-8">
        ...
        {isProductionDashboard ? (
          <section className="neon-surface neon-sweep rounded-2xl border border-cyan-200/80 bg-gradient-to-br from-white/95 via-slate-50/95 to-cyan-50/60 p-4">
            <div className="mb-3 flex items-center justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-wide text-orange-700">Production KPI</p><h2 className="text-lg font-black text-slate-900">ထုတ်လုပ်မှု အခြေအနေ</h2></div><Link href="/production" className="text-sm font-black text-orange-700 hover:underline">ထုတ်လုပ်မှု →</Link></div>
            <div className="dashboard-kpi-grid grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
              <Link href="/production" aria-label="ဗူး ထုတ်လုပ်မှု KPI အသေးစိတ်ကြည့်ရန်" className="neon-card neon-sweep flex h-full min-h-[128px] min-w-0 w-full flex-col items-start justify-between rounded-xl border border-orange-300 bg-orange-50/95 p-4 text-left shadow-sm transition-all hover:border-orange-500 hover:shadow-md focus:outline-none focus:ring-2 focus:ring-orange-300 sm:min-h-[158px]"><div><p className="text-sm font-black tracking-wide text-orange-800 sm:text-base">ဗူး ထုတ်လုပ်မှု</p><p className="mt-2 text-3xl font-black text-orange-950">{productionLoading || kpiDateLoading ? "ရယူနေသည်..." : `${productionSummary.totalPieces.toLocaleString()} ဗူး`}</p><p className="mt-1 text-sm font-bold text-orange-700">ကောင်းမွန် {productionSummary.goodPieces.toLocaleString()} · ပျက်စီး {productionSummary.wasteQuantity.toLocaleString()}</p></div><p className="pt-2 text-sm font-bold text-orange-700">အသေးစိတ်ကြည့်ရန် →</p></Link>
              <Link href="/production" aria-label="Tube ထုတ်လုပ်မှု KPI အသေးစိတ်ကြည့်ရန်" className="neon-card neon-sweep flex h-full min-h-[128px] min-w-0 w-full flex-col items-start justify-between rounded-xl border border-cyan-300 bg-cyan-50/95 p-4 text-left shadow-sm transition-all hover:border-cyan-500 hover:shadow-md focus:outline-none focus:ring-2 focus:ring-cyan-300 sm:min-h-[158px]"><div><p className="text-sm font-black tracking-wide text-cyan-800 sm:text-base">Tube ထုတ်လုပ်မှု</p><p className="mt-2 text-3xl font-black text-cyan-950">{productionLoading || kpiDateLoading ? "ရယူနေသည်..." : `${tubeProductionSummary.totalPacks.toLocaleString()} အိတ်`}</p><p className="mt-1 text-sm font-bold text-cyan-700">{tubeProductionSummary.totalPieces.toLocaleString()} pcs · အမျိုးအစား {tubeProductionSummary.rows.length} မျိုး</p></div><p className="pt-2 text-sm font-bold text-cyan-700">အသေးစိတ်ကြည့်ရန် →</p></Link>
              <Link href="/production-history" aria-label="ဗူး ထုတ်လုပ်မှုမှတ်တမ်း KPI အသေးစိတ်ကြည့်ရန်" className="neon-card neon-sweep flex h-full min-h-[128px] min-w-0 w-full flex-col items-start justify-between rounded-xl border border-blue-300 bg-blue-50/95 p-4 text-left shadow-sm transition-all hover:border-blue-500 hover:shadow-md focus:outline-none focus:ring-2 focus:ring-blue-300 sm:min-h-[158px]"><div><p className="text-sm font-black tracking-wide text-blue-800 sm:text-base">ဗူး ထုတ်လုပ်မှုမှတ်တမ်း</p><p className="mt-2 text-3xl font-black text-blue-950">{productionSummary.machineCount.toLocaleString()} စက်</p><p className="mt-1 text-sm font-bold text-blue-700">ယနေ့အသုံးပြုထားသော စက်များ</p></div><p className="pt-2 text-sm font-bold text-blue-700">မှတ်တမ်းကြည့်ရန် →</p></Link>
              <Link href="/tube-stock" aria-label="Tube လက်ကျန် KPI အသေးစိတ်ကြည့်ရန်" className="neon-card neon-sweep flex h-full min-h-[128px] min-w-0 w-full flex-col items-start justify-between rounded-xl border border-indigo-300 bg-indigo-50/95 p-4 text-left shadow-sm transition-all hover:border-indigo-500 hover:shadow-md focus:outline-none focus:ring-2 focus:ring-indigo-300 sm:min-h-[158px]"><div><p className="text-sm font-black tracking-wide text-indigo-800 sm:text-base">စက်ရုံ Tube လက်ကျန်</p><p className="mt-2 text-3xl font-black text-indigo-950">{dashboardKpiLoading || !dashboardKpi ? "ရယူနေသည်..." : `${factoryTubePacks.toLocaleString()} အိတ်`}</p><p className="mt-1 text-sm font-bold text-indigo-700">{dashboardKpiLoading || !dashboardKpi ? "ရယူနေသည်..." : `${factoryTubePieces.toLocaleString()} pcs`}</p></div><p className="pt-2 text-sm font-bold text-indigo-700">လက်ကျန်ကြည့်ရန် →</p></Link>
            </div>
          </section>
        ) : null}
        {!isLedgerView && !isSangEulDashboard && !isProductionDashboard ? (
