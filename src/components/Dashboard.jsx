...
export default function Dashboard({ view = "overview" }) {
  ...
  const [todaySummaryLoading, setTodaySummaryLoading] = useState(() => !Array.isArray(initialDashboardSnapshot?.todayPaymentsList) && !Array.isArray(initialDashboardSnapshot?.todayCashSales));
  ...
  const dashboardSummaryLoading = dashboardKpiLoading || kpiDateLoading || productionLoading || todaySummaryLoading;
  ...
  const paymentTargetLedgers = useMemo(() => {
    ...
    const credits = ledgers
      ...
      .map((credit) => ({
        ...credit,
        originalAmount: Math.max(0, Number(credit.remainingAmount ?? credit.amount ?? 0)),
        paidAmount: 0,
        remainingAmount: Math.max(0, Number(credit.remainingAmount ?? credit.amount ?? 0)),
      }))
  ...
  useEffect(() => {
    setFilteredLedgers(transactionRowsForFilter);
  }, [transactionRowsForFilter]);
      ...
      <div className="dashboard-content-stack mx-auto flex w-full min-w-0 max-w-7xl flex-col gap-6 overflow-x-clip px-3 py-3 sm:gap-6 sm:px-6 sm:py-6 lg:px-8">
        <header className="neon-surface neon-sweep min-w-0 rounded-2xl border border-cyan-200/80 bg-white/90 px-3 py-3 sm:px-5 sm:py-5">
          ...
          <div className="grid min-w-0 grid-cols-1 gap-4 md:gap-5 lg:grid-cols-[minmax(0,1fr)_auto_minmax(260px,1fr)] lg:items-center lg:gap-x-6">
            ...
            <div className="neon-control-deck order-3 grid w-full min-w-0 max-w-none grid-cols-2 items-center gap-1.5 rounded-xl border border-slate-200/80 bg-gradient-to-br from-slate-50/90 to-white p-1.5 shadow-sm lg:order-none lg:max-w-[360px] lg:justify-self-end">
              ...
              <Link
                href="/production"
                className="neon-menu-button neon-card-red flex min-h-16 min-w-0 w-full items-center justify-center gap-2 rounded-lg border border-red-400 bg-red-50 px-3 py-3 text-center text-base font-black leading-5 text-red-800 shadow-sm transition-colors hover:bg-red-100"
              ...
              <Link
                href="/ledger"
                className="neon-menu-button neon-card-blue flex min-h-16 min-w-0 w-full items-center justify-center gap-2 rounded-lg border border-blue-300 bg-blue-50 px-3 py-3 text-center text-base font-black leading-5 text-blue-800 shadow-sm transition-colors hover:bg-blue-100"
                title="ငွေရှင်းတမ်း"
              >
                <span className="inline-flex h-7 w-7 shrink-0 items-center justify-center text-lg leading-none" aria-hidden="true">💳</span>
                <span>ငွေရှင်းတမ်း</span>
              ...
              <Link
                href="/daily-summary"
                className="neon-menu-button neon-card-violet flex min-h-16 min-w-0 w-full items-center justify-center gap-2 rounded-lg border border-violet-300 bg-violet-50 px-3 py-3 text-center text-base font-black leading-5 text-violet-800 shadow-sm transition-colors hover:bg-violet-100"
              ...
              <Link
                href="/activity"
                className="neon-menu-button neon-card-yellow flex min-h-16 min-w-0 w-full items-center justify-center gap-2 rounded-lg border border-yellow-400 bg-yellow-50 px-3 py-3 text-center text-base font-black leading-5 text-yellow-800 shadow-sm transition-colors hover:bg-yellow-100"
              ...
              <Link
                href="/expenses"
                className="neon-menu-button neon-card-emerald flex min-h-16 min-w-0 w-full items-center justify-center gap-2 rounded-lg border border-emerald-300 bg-emerald-50 px-3 py-3 text-center text-base font-black leading-5 text-emerald-800 shadow-sm transition-colors hover:bg-emerald-100"
                title="အသုံးစားရိတ်"
              >
                <span className="inline-flex h-7 w-7 shrink-0 items-center justify-center text-lg leading-none" aria-hidden="true">💸</span>
                <span>အသုံးစားရိတ်</span>
              </Link>
              <Link href="/discounts" className="neon-menu-button neon-card-lime flex min-h-16 min-w-0 w-full items-center justify-center gap-2 rounded-lg border border-lime-400 bg-lime-50 px-3 py-3 text-center text-base font-black leading-5 text-lime-800 shadow-sm transition-colors hover:bg-lime-100" title="Customer လျှော့စျေးမှတ်တမ်း">
                <span className="inline-flex h-7 w-7 shrink-0 items-center justify-center text-lg leading-none" aria-hidden="true">💰</span>
              ...
              <button
                ...
                className="neon-menu-button neon-card-indigo flex min-h-16 min-w-0 w-full items-center justify-center gap-2 rounded-lg border border-indigo-300 bg-indigo-50/90 px-3 py-3 text-center text-base font-black leading-5 text-indigo-800 shadow-sm transition-colors hover:bg-indigo-100"
              >
                <span className="inline-flex h-7 w-7 shrink-0 items-center justify-center text-lg leading-none" aria-hidden="true">📊</span>
                <span>Settings</span>
        ...
        {!isLedgerView && !isCapStockDashboard ? (
          ...
          {dashboardSummaryLoading ? (
            <div className="mb-3 flex items-center gap-2 rounded-xl border border-cyan-200 bg-cyan-50 px-3 py-2 text-sm font-bold text-cyan-800" role="status" aria-live="polite">
              <span className="h-4 w-4 animate-spin rounded-full border-2 border-cyan-200 border-t-cyan-700" aria-hidden="true" />
              <span>Dashboard data များ ရယူနေပါသည် — ရရှိလာသည့်အပိုင်းများကို တဖြည်းဖြည်း ပြသနေပါသည်။</span>
            </div>
          ) : null}
          <div className="dashboard-kpi-grid grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
            ...
            >
              ...
              <p className="mt-2 text-2xl font-bold text-emerald-700">{todaySummaryLoading ? "ရယူနေသည်..." : todayTransactions}</p>
            ...
            >
              ...
              <div className="mt-2 space-y-0.5 text-sm font-bold text-orange-700 sm:text-base">
                <p>{productionLoading ? "Data ရယူနေသည်..." : `ကောင်းသောဗူး ${productionSummary.goodPieces.toLocaleString()} ဗူး`}</p>
            ...
            >
              <div>
                ...
                <p className="mt-1 text-sm font-bold text-fuchsia-700">{productionLoading ? "Data ရယူနေသည်..." : <>အလေးချိန် {packagingBagSummary.totalPackagingWeightLb.toLocaleString()} ပေါင်<br />{packagingBagSummary.totalPieces.toLocaleString()} ဗူး · အိတ်အရွယ် {packagingBagSummary.groups.length} မျိုး</>}</p>
              </div>
            ...
            >
              <div>
                ...
                <p className="mt-2 text-3xl font-black text-cyan-900">{productionLoading ? "ရယူနေသည်..." : `${tubeProductionSummary.totalPacks.toLocaleString()} အိတ်`}</p>
                <p className="mt-1 text-sm font-bold text-cyan-700">{productionLoading ? "Data ရယူနေသည်..." : `${tubeProductionSummary.totalPieces.toLocaleString()} pcs · အမျိုးအစား ${tubeProductionSummary.rows.length} မျိုး`}</p>
              </div>
            ...
            >
              <div>
                ...
                <p className="mt-2 text-2xl font-black text-slate-800">{bottleSalesLoading ? "ရယူနေသည်..." : `${Number(todayTotalBottleSales.totalBottles || 0).toLocaleString()} ဗူး`}</p>
                <div className="mt-1 space-y-0.5 text-sm font-bold text-slate-600">
                  <p>{bottleSalesLoading ? "ရောင်းစာရင်း ရယူနေသည်..." : "စုစုပေါင်း ရောင်းဗူး"}</p>
                  <p className="text-teal-700">{bottleSalesLoading ? "ငွေရပြီးစာရင်း ရယူနေသည်..." : `ငွေရပြီး ${Number(todayPaidBottleSales.totalBottles || 0) + Number(todayCashBottleSales.totalBottles || 0)} ဗူး`}</p>
                  <p className="text-violet-700">{bottleSalesLoading ? "အကြွေးရောင်းစာရင်း ရယူနေသည်..." : `အကြွေးရောင်း ${Number(todayCreditBottleSales.totalBottles || 0).toLocaleString()} ဗူး`}</p>
                </div>
              </div>
              <p className="pt-2 text-sm font-bold text-slate-600">အသေးစိတ် →</p>
            </Link>
            <DailySalesSummaryPanel selectedDate={selectedKpiDate} totalCount={todayCashCount} retailCount={todayCashRetail} wholesaleCount={todayCashWholesale} retailAmount={dashboardKpi?.retailAmount || 0} wholesaleAmount={dashboardKpi?.wholesaleAmount || 0} dateLoading={false} />
            <Link
              href="/tube-stock"
            ...
            >
              <div>
                ...
                <p className="mt-2 text-3xl font-black text-blue-950">{dashboardKpiLoading || kpiDateLoading || !dashboardKpi ? "ရယူနေသည်..." : `${factoryTubePacks.toLocaleString()} အိတ်`}</p>
                <p className="mt-1 text-sm font-bold text-blue-700">{dashboardKpiLoading || kpiDateLoading || !dashboardKpi ? "Canonical stock data ရယူနေသည်..." : `${factoryTubePieces.toLocaleString()} pcs · ထုတ်လုပ်ဝင်ပြီးနောက် ကျန်သော Tube`}</p>
              </div>
            ...
            >
              <div>
                ...
                <p className="mt-2 text-2xl font-black text-amber-950">{dashboardKpiLoading || kpiDateLoading || !dashboardKpi ? "ရယူနေသည်..." : `${factoryStockCards.toLocaleString()} ကဒ်`}</p>
                <p className="mt-1 text-sm font-bold text-amber-800">{dashboardKpiLoading || kpiDateLoading || !dashboardKpi ? "Canonical stock data ရယူနေသည်..." : "စက်ရုံထုတ်လုပ်ဝင်ပြီး ရောင်းထွက်သွားပြီးနောက် ကျန်သောကဒ်"}</p>
              </div>
          ...
          <section className="neon-surface neon-sweep rounded-2xl border border-cyan-200/80 bg-gradient-to-br from-white/95 via-slate-50/95 to-cyan-50/60 p-4">
            <div className="dashboard-kpi-grid grid grid-cols-1 gap-3 sm:grid-cols-2">
              {dashboardActorName === "ဇွဲဇွဲ" ? <>
                ...
                <Link href={`/packaging-bag-report?date=${encodeURIComponent(selectedKpiDate)}`} aria-label={`${selectedKpiDate} ထုပ်ပိုး အိတ်ခွံ အသေးစိတ်ကြည့်ရန်`} className="neon-card neon-sweep flex h-full min-h-[128px] min-w-0 w-full flex-col items-start justify-between rounded-xl border border-fuchsia-200 bg-fuchsia-50/90 p-4 text-left shadow-sm transition-all hover:border-fuchsia-400 hover:shadow-md focus:outline-none focus:ring-2 focus:ring-fuchsia-300 sm:min-h-[170px]"><div><p className="text-sm font-black tracking-wide text-fuchsia-700 sm:text-base">{selectedKpiIsToday ? "ယနေ့" : selectedKpiDate} ထုပ်ပိုး အိတ်ခွံ</p><p className="mt-2 text-2xl font-black text-fuchsia-900">{productionLoading ? "ရယူနေသည်..." : `${packagingBagSummary.totalPackagingPieces.toLocaleString()} လုံး`}</p><p className="mt-1 text-sm font-bold text-fuchsia-700">{productionLoading ? "Data ရယူနေသည်..." : <>အလေးချိန် {packagingBagSummary.totalPackagingWeightLb.toLocaleString()} ပေါင်<br />{packagingBagSummary.totalPieces.toLocaleString()} ဗူး · အိတ်အရွယ် {packagingBagSummary.groups.length} မျိုး</>}</p></div><p className="pt-2 text-sm font-bold text-fuchsia-700">တွက်ချက်မှုကြည့်ရန် →</p></Link>
                <Link href={`/packaging-bag-stock?date=${encodeURIComponent(selectedKpiDate)}`} aria-label="ထုပ်ပိုး အိတ်ခွံ လက်ကျန် အသေးစိတ်ကြည့်ရန်" className="neon-card neon-sweep flex h-full min-h-[128px] min-w-0 w-full flex-col items-start justify-between rounded-xl border border-cyan-300 bg-cyan-50/95 p-4 text-left shadow-sm transition-all hover:border-cyan-500 hover:shadow-md focus:outline-none focus:ring-2 focus:ring-cyan-300 sm:min-h-[170px]"><div><p className="text-sm font-black tracking-wide text-cyan-800 sm:text-base">ထုပ်ပိုး အိတ်ခွံ လက်ကျန်</p><p className="mt-2 text-2xl font-black text-cyan-950">{dashboardKpiLoading || kpiDateLoading || !dashboardKpi ? "ရယူနေသည်..." : `${factoryPackagingBagPieces.toLocaleString()} လုံး`}</p><p className="mt-1 text-sm font-bold text-cyan-700">အသစ်ဝင် − သုံးစွဲ = လက်ကျန်</p></div><p className="pt-2 text-sm font-bold text-cyan-700">တွက်ချက်မှုကြည့်ရန် →</p></Link>
                ...
                <div className="mt-5 grid grid-cols-1 gap-4 sm:gap-6 items-start">
                  <div className="w-full rounded-xl border border-slate-200 bg-white p-3 shadow-sm sm:p-5">
                    ...
                    <form data-save-review-handled="true" className="mt-3 space-y-3" onSubmit={createLedgerTransaction}>
                      ...
                      >
                        {isSubmitting ? "သိမ်းဆည်းနေသည်..." : editingTransaction ? "ပြင်ဆင်ပြီး သိမ်းမည်" : "စာရင်းသိမ်းမည်"}
                      </button>
                    </form>
                  </div>
                </div>
              ...
              {reconciliationDetailType === "balance" ? (
                ...
                <div className="space-y-2">
                  {dashboardReconciliation.settlementExceptions.map((row, index) => {
                    const isPrepayment = row.type === "LINKED_PREPAYMENT";
