...
export default function DailySalesSummaryPanel({ selectedDate = "", totalCount = 0, retailCount = 0, wholesaleCount = 0, retailAmount = 0, wholesaleAmount = 0, dateLoading = false, fullPage = false }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/daily-sales-summary?date=${encodeURIComponent(historyDate)}`, {
      ...
      const body = await response.json().catch(() => ({}));
      if (!response.ok || !body.ok) throw new Error(body.error || "နေ့စဉ် history data မရသေးပါ။");
  ...
  const tableRows = useMemo(() => {
    ...
    const chronologicalRows = [...tableSummary.rows].sort((a, b) => a.date.localeCompare(b.date));
    const rowsWithCumulative = chronologicalRows.map((row) => {
      const displayRow = dateKey(row.date) === dateKey(historyDate) && dateKey(historyDate) === dateKey(date)
        ? {
            ...row,
  ...
  return (
    <>
      ...
      >
        ...
        <p className="mt-2 flex min-h-8 items-center text-2xl font-black text-violet-900">{!fullPage && dateLoading ? formatMoney(dailyTotal) : loading || dateLoading ? "ရယူနေသည်..." : formatMoney(dailyTotal)}</p>
        <div className="mt-2 min-h-[4.5rem] space-y-0.5 text-sm font-bold text-violet-700 sm:text-base">
          <p>{!fullPage && dateLoading ? `လက်ငင်း ${totalCount} ခု` : loading || dateLoading ? "ရယူနေသည်..." : `လက်ငင်း ${totalCount} ခု`}</p>
          <p>{!fullPage && dateLoading ? `လက်လီ ${retailCount} ခု` : loading || dateLoading ? "ရယူနေသည်..." : `လက်လီ ${retailCount} ခု`}</p>
          <p>{!fullPage && dateLoading ? `လက်ကား ${wholesaleCount} ခု` : loading || dateLoading ? "ရယူနေသည်..." : `လက်ကား ${wholesaleCount} ခု`}</p>
        </div>
      ...
      {isOpen ? (
        <div className={fullPage ? "app-page-main min-h-screen" : "fixed inset-0 z-[120] flex items-start justify-center bg-slate-950/35 p-2 backdrop-blur-[2px] sm:items-center sm:p-5"} style={fullPage ? undefined : { paddingTop: "max(0.5rem, env(safe-area-inset-top))" }} role={fullPage ? undefined : "dialog"} aria-modal={fullPage ? undefined : "true"} aria-labelledby="daily-sales-summary-title">
          <section className={fullPage ? "mx-auto w-full max-w-[96rem] px-3 sm:px-6 lg:px-8" : "relative max-h-[calc(100dvh-1rem)] w-full max-w-3xl overflow-y-auto rounded-2xl border border-indigo-200 bg-white p-3 shadow-2xl sm:max-h-[94vh] sm:p-6"}>
            ...
            {showOpeningForm && (
              <div className="mt-3 rounded-xl border border-indigo-200 bg-indigo-50/40 p-4">
                ...
                <div className="mt-3 grid gap-3 sm:grid-cols-3">
                  ...
                  <label className="space-y-1">
                    <span className="text-[11px] font-bold text-indigo-700">ဘယ်ရက်အထိ</span>
            ...
            {loading ? <p className="mt-4 rounded-xl border border-indigo-100 bg-indigo-50 p-4 text-center text-sm text-indigo-800">အချက်အလက် ရယူနေပါသည်...</p> : null}
            ...
            <div className="mt-4 grid grid-cols-2 gap-2 sm:gap-3 sm:items-stretch">
              {[INPUT_FIELDS[0], INPUT_FIELDS[2], INPUT_FIELDS[1], INPUT_FIELDS[3]].map((field) => (
                <label key={field.key} className={`flex h-full flex-col rounded-xl border p-3 ${toneClasses(field.tone)}`}>
                  <span className="block text-sm font-semibold leading-6 sm:text-base">{field.label}</span>
              ...
              <div className="flex h-full flex-col rounded-xl border border-rose-200 bg-rose-50 p-3.5">
                <p className="text-xs font-semibold text-rose-800">ယနေ့ လက်လီ + လက်ကား ရောင်းရငွေ</p>
                    <p className="mt-1 text-2xl font-bold text-rose-900 sm:text-3xl">{formatMoney(dailyTotal)}</p>
              ...
              <div className="flex h-full flex-col rounded-xl border border-rose-200 bg-rose-50 p-3.5">
                <p className="text-xs font-semibold text-rose-800">တစ်နေ့တာ ငွေသား</p>
                    <p className="mt-1 text-2xl font-bold text-rose-900 sm:text-3xl">{formatMoney(cashDailyTotal)}</p>
