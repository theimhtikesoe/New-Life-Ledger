    ...
    <>
      ...
      >
        ...
        <div className="mt-2 min-h-[4.5rem] space-y-0.5 text-sm font-bold text-violet-700 sm:text-base">
          <p>{loading || dateLoading ? "လက်ငင်းစာရင်း ရယူနေသည်..." : `လက်ငင်း ${totalCount} ခု`}</p>
          <p>{loading || dateLoading ? "လက်လီစာရင်း ရယူနေသည်..." : `လက်လီ ${retailCount} ခု`}</p>
          <p>{loading || dateLoading ? "လက်ကားစာရင်း ရယူနေသည်..." : `လက်ကား ${wholesaleCount} ခု`}</p>
        </div>
      ...
      {isOpen ? (
        <div className={fullPage ? "app-page-main min-h-screen" : "fixed inset-0 z-[120] flex items-start justify-center bg-slate-950/35 p-2 backdrop-blur-[2px] sm:items-center sm:p-5"} style={fullPage ? undefined : { paddingTop: "max(0.5rem, env(safe-area-inset-top))" }} role={fullPage ? undefined : "dialog"} aria-modal={fullPage ? undefined : "true"} aria-labelledby="daily-sales-summary-title">
          <section data-layout-version={fullPage ? "shared-header-width-v3" : undefined} className={fullPage ? "shared-report-route-inner mx-auto w-full px-3 sm:px-6 lg:px-8" : "relative max-h-[calc(100dvh-1rem)] w-full max-w-3xl overflow-y-auto rounded-2xl border border-indigo-200 bg-white p-3 shadow-2xl sm:max-h-[94vh] sm:p-6"} style={fullPage ? { width: "100%", maxWidth: "80rem", marginInline: "auto" } : undefined}>
