...
export default function PackagingBagReportPage() {
  ...
  return (
    <main data-layout-version="shared-header-width-v4" className="shared-report-route-main mx-auto w-full space-y-4 px-0 pb-8">
      <section className="rounded-2xl border border-cyan-200 bg-white p-4 shadow-sm sm:p-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div><p className="text-sm font-bold text-cyan-700">ဗူးထွက်ရှိမှုမှတ်တမ်းကို အိတ်ခွံအရွယ်အစားအလိုက် လုံးရေပြန်တွက်ထားသော စာရင်း</p><p className="mt-1 text-xs font-bold text-slate-500">အရွယ်အစားတစ်ခုချင်းစီအလိုက် တစ်အိတ်ထဲပါသော လုံးရေ၊ အလေးချိန်ဖြင့် တွက်ချက်ထားပါသည်။</p></div>
      ...
      <section className="grid gap-3 sm:grid-cols-4"><div className="rounded-xl border border-cyan-200 bg-cyan-50 p-4"><p className="text-sm font-bold text-cyan-700">ယနေ့ အိတ်ခွံလုံး စုစုပေါင်း</p><p className="mt-1 text-2xl font-black text-cyan-900">{loading ? "—" : `${formatNumber(report.totalPackagingPieces)} လုံး`}</p></div><div className="rounded-xl border border-violet-200 bg-violet-50 p-4"><p className="text-sm font-bold text-violet-700">ယနေ့ အလေးချိန်</p><p className="mt-1 text-2xl font-black text-violet-900">{loading ? "—" : `${formatNumber(report.totalPackagingWeightLb)} ပေါင်`}</p></div><div className="rounded-xl border border-emerald-200 bg-emerald-50 p-4"><p className="text-sm font-bold text-emerald-700">ယနေ့ ထုပ်ပိုးသည့် ဗူး</p><p className="mt-1 text-2xl font-black text-emerald-900">{loading ? "—" : `${formatNumber(report.totalPieces)} ဗူး`}</p></div><div className="rounded-xl border border-slate-200 bg-slate-50 p-4"><p className="text-sm font-bold text-slate-700">အိတ်ခွံအရွယ်အစား</p><p className="mt-1 text-2xl font-black text-slate-900">{loading ? "—" : `${formatNumber(report.groups.length)} မျိုး`}</p></div></section>
      {loading ? <div className="rounded-2xl border border-dashed p-10 text-center font-bold text-slate-500">ထုပ်ပိုးအိတ်ခွံစာရင်း ရယူနေသည်...</div> : report.groups.length === 0 ? <div className="rounded-2xl border border-dashed p-10 text-center font-bold text-slate-500">{date} အတွက် တွက်ချက်နိုင်သော ဗူးထွက်ရှိမှု မရှိသေးပါ။</div> : <section className="space-y-3">{report.groups.map((group) => <article key={group.bagSize} className="rounded-2xl border border-cyan-200 bg-white p-4 shadow-sm sm:p-5"><div className="flex flex-wrap items-center justify-between gap-2"><div><h2 className="text-xl font-black text-slate-900">{group.label} အိတ်ခွံ</h2><p className="text-xs font-bold text-slate-500">{group.piecesPerBag} လုံး/အိတ် · {formatNumber(group.weightLb)} ပေါင်/အိတ်</p></div><div className="rounded-xl bg-cyan-600 px-4 py-2 text-right text-white"><p className="text-xs font-bold">အိတ်ခွံလုံး အရေအတွက်</p><p className="text-xl font-black">{formatNumber(group.packagingPieces)} လုံး</p><p className="text-xs font-bold">{formatNumber(group.packagingWeightLb)} ပေါင်</p></div></div><div className="mt-3 overflow-x-auto rounded-xl border border-slate-100"><table className="w-full min-w-[520px] text-left text-sm"><thead className="bg-slate-100 text-slate-700"><tr><th className="px-3 py-2 font-black">ဗူးအမျိုးအစား</th><th className="px-3 py-2 font-black">ဆံ့</th><th className="px-3 py-2 text-right font-black">ကဒ်</th><th className="px-3 py-2 text-right font-black">ဗူး</th></tr></thead><tbody>{group.items.map((item) => <tr key={item.key} className="border-t border-slate-100"><td className="px-3 py-2 font-bold">{item.label}</td><td className="px-3 py-2 font-bold">{formatNumber(item.capacity)} ဆံ့</td><td className="px-3 py-2 text-right font-black">{formatNumber(item.quantity)} {item.unit}</td><td className="px-3 py-2 text-right font-black text-emerald-700">{formatNumber(item.quantity * item.capacity)}</td></tr>)}</tbody></table></div></article>)}</section>}
