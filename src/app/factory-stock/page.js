...
function stockStatus(item) {
  ...
  if (Number(item.unrecordedOpeningStockCards || item.unrecordedOpeningStockBottles || 0) > 0) return { label: "မထည့်ရသေးသော နဂို Stock သုံးပြီး", className: "bg-amber-100 text-amber-800" };
  ...
  return (
    <main className="app-page-main">
      <div className="app-page-container app-page-surface space-y-4 pt-5 sm:pt-6">
        <button type="button" onClick={() => setUsageOpen(true)} className="flex w-full items-center justify-between gap-3 rounded-2xl border border-rose-200 bg-rose-50 p-4 text-left shadow-sm"><div><p className="text-sm font-black text-rose-800">ယနေ့ ဗူးသုံးစွဲမှု</p><p className="mt-1 text-2xl font-black text-rose-950">{number(dailyBottlePieces)} ဗူး</p></div><span className="text-sm font-black text-rose-700">အသေးစိတ်ကြည့်ရန် →</span></button>
        ...
        {!loading && bottleSummary.length ? <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
          <div className="hidden overflow-x-auto sm:block"><table className="w-full min-w-[860px] text-sm"><thead className="bg-slate-50 text-left text-xs font-black text-slate-500"><tr><th className="px-4 py-3">ဗူးအမျိုးအစား</th><th className="px-4 py-3 text-right">ဆံ့</th><th className="px-4 py-3 text-right">ထုတ်လုပ်ဝင်</th><th className="px-4 py-3 text-right">သုံးစွဲ/ရောင်းထွက်</th><th className="px-4 py-3 text-right">မထည့်ရသေးသော နဂို Stock သုံးပြီး</th><th className="px-4 py-3 text-right">Net Stock Change</th><th className="px-4 py-3">အခြေအနေ</th><th className="px-4 py-3">အသေးစိတ်</th></tr></thead><tbody>{bottleSummary.map((item) => { const status = stockStatus(item); return <tr key={item.productKey} className="border-t border-slate-100"><td className="px-4 py-3 font-black text-slate-900">{item.productName}</td><td className="px-4 py-3 text-right font-bold text-slate-600">{number(item.capacity)}</td><td className="px-4 py-3 text-right font-black text-emerald-700">+{number(item.productionCards)} {item.stockType === "TUBE" ? `အိတ် / ${number(item.productionBottles)} pcs` : "ကဒ်"}</td><td className="px-4 py-3 text-right font-black text-rose-700">-{number(item.stockType === "TUBE" ? item.usedBottles : item.soldCards)} {item.stockType === "TUBE" ? "pcs သုံး" : "ကဒ်ရောင်း"}</td><td className="px-4 py-3 text-right font-black text-amber-800">{number(item.stockType === "TUBE" ? item.unrecordedOpeningStockBottles : item.unrecordedOpeningStockCards)} {item.stockType === "TUBE" ? "pcs" : "ကဒ်"}</td><td className={`px-4 py-3 text-right text-base font-black ${(item.stockType === "TUBE" ? item.currentBottles : item.currentCards) < 0 ? "text-rose-700" : "text-slate-900"}`}>{number(item.stockType === "TUBE" ? item.currentBottles : item.currentCards)} {item.stockType === "TUBE" ? "pcs" : "ကဒ်"}</td><td className="px-4 py-3"><span className={`inline-flex rounded-full px-2 py-1 text-xs font-black ${status.className}`}>{status.label}</span></td><td className="px-4 py-3"><button type="button" onClick={() => openDetails(item.productKey)} className="rounded-lg border border-orange-200 bg-orange-50 px-3 py-2 text-xs font-black text-orange-800">ကြည့်</button></td></tr>; })}</tbody></table></div>
          <div className="divide-y divide-slate-100 sm:hidden">{bottleSummary.map((item) => { const status = stockStatus(item); return <article key={item.productKey} className="space-y-3 p-4"><div className="flex items-start justify-between gap-3"><div><h2 className="font-black text-slate-900">{item.productName}</h2><p className="text-xs font-bold text-slate-500">{number(item.capacity)} ဆံ့</p></div><span className={`rounded-full px-2 py-1 text-xs font-black ${status.className}`}>{status.label}</span></div><div className="grid grid-cols-2 gap-2 text-sm"><p className="rounded-lg bg-emerald-50 p-2 font-bold text-emerald-800">ဝင် +{number(item.productionCards)} {item.stockType === "TUBE" ? `အိတ် / ${number(item.productionBottles)} pcs` : "ကဒ်"}</p><p className="rounded-lg bg-rose-50 p-2 font-bold text-rose-800">{item.stockType === "TUBE" ? `သုံး -${number(item.usedBottles)} pcs` : `ထွက် -${number(item.soldCards)} ကဒ်`}</p><p className="rounded-lg bg-orange-50 p-2 font-bold text-orange-800">မထည့်ရသေးသော နဂို Stock သုံးပြီး {number(item.stockType === "TUBE" ? item.unrecordedOpeningStockBottles : item.unrecordedOpeningStockCards)} {item.stockType === "TUBE" ? "pcs" : "ကဒ်"}</p><p className={`rounded-lg bg-slate-100 p-2 font-black ${((item.stockType === "TUBE" ? item.currentBottles : item.currentCards) < 0) ? "text-rose-700" : "text-slate-900"}`}>Net Stock Change {number(item.stockType === "TUBE" ? item.currentBottles : item.currentCards)} {item.stockType === "TUBE" ? "pcs" : "ကဒ်"}</p></div><button type="button" onClick={() => openDetails(item.productKey)} className="min-h-10 w-full rounded-lg border border-orange-200 bg-orange-50 px-3 py-2 text-sm font-black text-orange-800">ကြည့်</button></article>; })}</div>
        </section> : null}
      </div>
      {usageOpen ? <div className="fixed inset-0 z-[150] flex items-end justify-center bg-slate-950/60 p-4 backdrop-blur-sm sm:items-center" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setUsageOpen(false); }}><section role="dialog" aria-modal="true" aria-labelledby="daily-bottle-usage-title" className="max-h-[85dvh] w-full overflow-hidden rounded-2xl bg-white shadow-2xl sm:max-w-xl"><div className="flex items-start justify-between gap-3 border-b border-rose-100 bg-rose-50 px-4 py-4"><div><h2 id="daily-bottle-usage-title" className="text-lg font-black text-slate-900">ယနေ့ ဗူးသုံးစွဲမှု</h2><p className="mt-1 text-sm font-bold text-rose-800">စုစုပေါင်း {number(dailyBottlePieces)} ဗူး</p></div><button type="button" onClick={() => setUsageOpen(false)} className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm font-bold text-slate-600">ပိတ်</button></div><div className="max-h-[65dvh] space-y-2 overflow-y-auto p-4">{dailyBottleUsage.length ? dailyBottleUsage.map((row) => <div key={`${row.name}-${row.capacity}`} className="flex items-center justify-between gap-3 rounded-xl border border-rose-100 bg-rose-50/60 p-3"><div><p className="font-black text-slate-900">{row.name}</p><p className="text-xs font-bold text-slate-500">{number(row.capacity)} ဆံ့ · {number(row.cards)} ကဒ်</p></div><p className="text-base font-black text-rose-700">-{number(row.bottles)} ဗူး</p></div>) : <p className="py-8 text-center font-bold text-slate-500">ယနေ့ ဗူးသုံးစွဲမှု မရှိသေးပါ။</p>}</div></section></div> : null}
