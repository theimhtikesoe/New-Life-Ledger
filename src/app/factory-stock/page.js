    ...
    <main className="app-page-main">
      <div className="app-page-container app-page-surface space-y-4 pt-5 sm:pt-6">
        ...
        {!loading && bottleSummary.length ? <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
          <div className="hidden overflow-x-auto sm:block"><table className="w-full min-w-[900px] text-sm"><thead className="bg-slate-50 text-left text-xs font-black text-slate-500"><tr><th className="px-4 py-3">ဗူးအမျိုးအစား</th><th className="px-4 py-3 text-right">ဆံ့</th><th className="px-4 py-3 text-right">ထုတ်လုပ်ဝင်</th><th className="px-4 py-3 text-right">သုံးစွဲ/ရောင်းထွက်</th><th className="px-4 py-3 text-right">စာရင်းညှိ</th><th className="px-4 py-3 text-right">နဂိုရှိ Stock</th><th className="px-4 py-3 text-right">လက်ကျန်</th><th className="px-4 py-3">အခြေအနေ</th><th className="px-4 py-3">အသေးစိတ်</th></tr></thead><tbody>{bottleSummary.map((item) => { const status = stockStatus(item); return <tr key={item.productKey} className="border-t border-slate-100"><td className="px-4 py-3 font-black text-slate-900">{item.productName}</td><td className="px-4 py-3 text-right font-bold text-slate-600">{number(item.capacity)}</td><td className="px-4 py-3 text-right font-black text-emerald-700">+{number(item.productionCards)} {item.stockType === "TUBE" ? `အိတ် / ${number(item.productionBottles)} pcs` : "ကဒ်"}</td><td className="px-4 py-3 text-right font-black text-rose-700">-{number(item.stockType === "TUBE" ? item.usedBottles : item.soldCards)} {item.stockType === "TUBE" ? "pcs သုံး" : "ကဒ်ရောင်း"}</td><td className="px-4 py-3 text-right font-black text-amber-700">{item.adjustmentCards > 0 ? "+" : ""}{number(item.stockType === "TUBE" ? item.adjustmentBottles : item.adjustmentCards)} {item.stockType === "TUBE" ? "pcs" : "ကဒ်"}</td><td className="px-4 py-3 text-right font-black text-amber-800">{number(item.stockType === "TUBE" ? item.openingStockBottles : item.openingStockCards)} {item.stockType === "TUBE" ? "pcs" : "ကဒ်"}</td><td className={`px-4 py-3 text-right text-base font-black ${(item.stockType === "TUBE" ? item.currentBottles : item.currentCards) < 0 ? "text-rose-700" : "text-slate-900"}`}>{number(item.stockType === "TUBE" ? item.currentBottles : item.currentCards)} {item.stockType === "TUBE" ? "pcs" : "ကဒ်"}</td><td className="px-4 py-3"><span className={`inline-flex rounded-full px-2 py-1 text-xs font-black ${status.className}`}>{status.label}</span></td><td className="px-4 py-3"><button type="button" onClick={() => openDetails(item.productKey)} className="rounded-lg border border-orange-200 bg-orange-50 px-3 py-2 text-xs font-black text-orange-800">ကြည့်</button></td></tr>; })}</tbody></table></div>
          <div className="divide-y divide-slate-100 sm:hidden">{bottleSummary.map((item) => { const status = stockStatus(item); return <article key={item.productKey} className="space-y-3 p-4"><div className="flex items-start justify-between gap-3"><div><h2 className="font-black text-slate-900">{item.productName}</h2><p className="text-xs font-bold text-slate-500">{number(item.capacity)} ဆံ့</p></div><span className={`rounded-full px-2 py-1 text-xs font-black ${status.className}`}>{status.label}</span></div><div className="grid grid-cols-2 gap-2 text-sm"><p className="rounded-lg bg-emerald-50 p-2 font-bold text-emerald-800">ဝင် +{number(item.productionCards)} {item.stockType === "TUBE" ? `အိတ် / ${number(item.productionBottles)} pcs` : "ကဒ်"}</p><p className="rounded-lg bg-rose-50 p-2 font-bold text-rose-800">{item.stockType === "TUBE" ? `သုံး -${number(item.usedBottles)} pcs` : `ထွက် -${number(item.soldCards)} ကဒ်`}</p><p className="rounded-lg bg-amber-50 p-2 font-bold text-amber-800">ညှိ {number(item.stockType === "TUBE" ? item.adjustmentBottles : item.adjustmentCards)} {item.stockType === "TUBE" ? "pcs" : "ကဒ်"}</p><p className="rounded-lg bg-orange-50 p-2 font-bold text-orange-800">နဂိုရှိ Stock {number(item.stockType === "TUBE" ? item.openingStockBottles : item.openingStockCards)} {item.stockType === "TUBE" ? "pcs" : "ကဒ်"}</p><p className="rounded-lg bg-slate-100 p-2 font-black text-slate-900">လက်ကျန် {number(item.stockType === "TUBE" ? item.currentBottles : item.currentCards)} {item.stockType === "TUBE" ? "pcs" : "ကဒ်"}</p></div><button type="button" onClick={() => openDetails(item.productKey)} className="min-h-10 w-full rounded-lg border border-orange-200 bg-orange-50 px-3 py-2 text-sm font-black text-orange-800">ကြည့်</button></article>; })}</div>
        </section> : null}
      </div>
