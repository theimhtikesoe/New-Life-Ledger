...
function CustomerRow({ customer, onToggleReminder, onOpenSettlement, updating }) {
  ...
  return (
    <article
      className="cursor-pointer rounded-xl border border-slate-200 bg-white p-4 shadow-sm transition hover:border-cyan-300 hover:shadow-md focus:outline-none focus:ring-2 focus:ring-cyan-400 focus:ring-offset-2"
      role="link"
      tabIndex={0}
      aria-label={`${customer.name} ငွေစာရင်းအသေးစိတ်ကြည့်ရန်`}
      onClick={(event) => {
        if (event.target.closest("a, button")) return;
        router.push(`/ledger?customerId=${encodeURIComponent(customer.id)}`);
      }}
      onKeyDown={(event) => {
...
export default function BalanceDetailPage() {
  ...
  return (
    <main className="app-page-main">
      <div className="app-page-container app-page-surface">
        <section className="page-toolbar">
          <div className="page-toolbar-row items-start">
            <p className="page-toolbar-description">ဘယ်သူ့ဆီက ငွေရရန်ရှိသလဲ၊ ဘယ်သူက ငွေပိုချေထားသလဲ အသေးစိတ်ကြည့်ရန်</p>
            <nav aria-label="စာမျက်နှာများ" className="page-toolbar-controls w-full justify-start text-xs font-semibold sm:w-auto">
              ...
              <Link href="/discounts" className="rounded-full bg-orange-50 px-3 py-2 text-orange-700 hover:bg-orange-100">Discount History</Link>
            </nav>
          </div>
        </section>
      ...
      {settlementCustomer ? <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 p-4"><div className="w-full max-w-lg rounded-2xl bg-white p-5 shadow-2xl"><div className="flex items-center justify-between"><h2 className="text-xl font-black">{settlementCustomer.name} — မြေပြင်ငွေချေ</h2><button onClick={() => setSettlementCustomer(null)} className="text-2xl">×</button></div><p className="mt-1 text-sm text-slate-500">ငွေချေပြီးကြောင်း မှတ်မည့် အကြွေးနှင့် အမှန်တကယ် ချေငွေကို သီးခြားရွေး/ထည့်ပါ။</p>{settlementLoading ? <div className="mt-4 rounded-xl bg-slate-50 p-3 text-center text-sm font-bold text-slate-600">ယခင်မှတ်တမ်းများ ရယူနေသည်...</div> : null}<label className="mt-4 block text-sm font-bold">အကြွေးဟောင်း ရွေးရန်<select disabled={settlementLoading} value={settlementLedgerId} onChange={e => setSettlementLedgerId(e.target.value)} className="mt-1 w-full rounded-xl border p-3"><option value="">အကြွေးအားလုံး / သီးခြားမရွေး</option>{settlementTransactions.filter(t => t.type === "CREDIT" || !t.type).map(t => <option key={t.id} value={t.id}>{new Date(t.date).toLocaleDateString("en-GB")} · {Number(t.amount || 0).toLocaleString()} Ks · {t.note || "အကြွေး"}</option>)}</select></label><label className="mt-3 block text-sm font-bold">ဘာနဲ့ ချေသလဲ<select value={settlementMethod} onChange={e => setSettlementMethod(e.target.value)} className="mt-1 w-full rounded-xl border p-3"><option>ငွေသား</option><option>KPay</option><option>Wave</option><option>ဘဏ်လွှဲ</option><option>အခြား</option></select></label><label className="mt-3 block text-sm font-bold">တကယ် ချေတဲ့ငွေ (Ks)<input inputMode="numeric" value={settlementAmount} onChange={e => setSettlementAmount(e.target.value.replace(/[^0-9,]/g, ""))} className="mt-1 w-full rounded-xl border p-3" /></label><label className="mt-3 block text-sm font-bold">အသေးစိတ်မှတ်ချက်<textarea value={settlementNote} onChange={e => setSettlementNote(e.target.value)} className="mt-1 w-full rounded-xl border p-3" rows="2" /></label>{settlementCustomer.settlements?.length ? <div className="mt-4 rounded-xl border border-emerald-200 bg-emerald-50 p-3"><p className="text-sm font-black text-emerald-900">ယခင် မြေပြင်ငွေချေမှတ်တမ်း</p>{settlementCustomer.settlements.map((item) => <div key={item.id} className="mt-2 rounded-lg bg-white p-2 text-xs text-slate-700"><p><b>{Number(item.amount || 0).toLocaleString()} Ks</b> · {item.paymentMethod} · {new Date(item.settledAt).toLocaleString("en-GB")}</p>{item.ledgerId ? <p className="mt-1">ရွေးထားသော အကြွေး ID — {item.ledgerId}</p> : null}{item.note ? <p className="mt-1">{item.note}</p> : null}</div>)}<button type="button" onClick={undoLatestSettlement} disabled={settlementSaving} className="mt-3 text-xs font-bold text-rose-700 underline">အမှားနှိပ်မိလျှင် နောက်ဆုံးမှတ်ချက်ကို ဖြုတ်မည်</button></div> : null}<div className="mt-5 flex gap-3"><button onClick={() => setSettlementCustomer(null)} className="flex-1 rounded-xl bg-slate-100 py-3 font-bold">မလုပ်တော့</button><button disabled={settlementSaving} onClick={saveSettlement} className="flex-1 rounded-xl bg-emerald-600 py-3 font-black text-white">{settlementSaving ? "သိမ်းနေသည်..." : "ငွေချေပြီး မှတ်မည်"}</button></div></div></div> : null}
      </div>
    </main>
  );
}
