...
function formatLedgerReference(ledgerId, transactions = []) {
  if (!ledgerId) return "အကြွေးအားလုံး";
  const ledger = transactions.find((row) => String(row.id) === String(ledgerId));
  if (!ledger) return "ချိတ်ထားသော အကြွေးစာရင်း";
  const date = new Date(ledger.date);
  const dateLabel = Number.isNaN(date.getTime()) ? "ရက်စွဲမသိ" : date.toLocaleDateString("en-GB");
  return `${dateLabel} · ${formatMoney(ledger.amount)} · အမှတ် ${String(ledger.id).slice(0, 8).toUpperCase()}`;
}
