  ...
  return (
    <main className="app-page-main">
      <div className="app-page-container space-y-4">
        ...
        {!loading && data ? <>
          <details className="group rounded-2xl border border-orange-200 bg-orange-50/60 p-4 shadow-sm sm:p-5">
            <summary className="cursor-pointer list-none text-xl font-black text-orange-950 marker:hidden"><span className="mr-2 inline-block transition-transform group-open:rotate-90">▶</span>Tube အလိုက် စုစုပေါင်းဗူး</summary><p className="mt-1 text-sm font-semibold text-orange-800">13g / 16g / 24g Tube နှင့် အဖြူ / အပြာ အလိုက် ခွဲပြထားပါသည်။</p><div className="mt-3 overflow-x-auto"><table className="w-full min-w-[760px] text-sm"><thead><tr className="border-b-2 border-orange-200 text-left text-xs text-orange-800"><th className="px-2 py-3">Tube အမျိုးအစား</th><th className="px-2 py-3">အရောင်</th><th className="px-2 py-3">သတ်မှတ်ချက်</th><th className="px-2 py-3 text-right">ကဒ်</th><th className="px-2 py-3 text-right">စုစုပေါင်းဗူး</th><th className="px-2 py-3 text-right">ငွေ</th></tr></thead><tbody>{tubeItems.length ? tubeItems.map((item) => <tr key={`${item.grams}-${item.color}-${item.tubeType}`} className="border-b border-orange-100"><td className="px-2 py-3 font-black">{item.grams}</td><td className="px-2 py-3 font-bold">{item.color}</td><td className="px-2 py-3">{item.tubeType}</td><td className="px-2 py-3 text-right">{number(item.cardCount)}</td><td className="px-2 py-3 text-right font-black text-orange-700">{number(item.bottleCount)}</td><td className="px-2 py-3 text-right font-bold">{money(item.totalAmount)}</td></tr>) : <tr><td colSpan="6" className="px-2 py-6 text-center font-bold text-slate-500">ဒီလအတွက် Tube mapping ပါသော ရောင်းစာရင်း မရှိသေးပါ။</td></tr>}</tbody></table></div></details>
          <details className="group rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
            <summary className="cursor-pointer list-none text-xl font-black text-slate-900 marker:hidden"><span className="mr-2 inline-block transition-transform group-open:rotate-90">▶</span>ရက်စွဲအလိုက် စုစုပေါင်း</summary><div className="mt-3 overflow-x-auto"><table className="w-full min-w-[680px] text-sm"><thead><tr className="border-b-2 border-slate-200 text-left text-xs text-slate-500"><th className="px-2 py-3">ရက်စွဲ</th><th className="px-2 py-3 text-right">Customer</th><th className="px-2 py-3 text-right">ရောင်းဗူး</th><th className="px-2 py-3 text-right">ရောင်းတန်ဖိုး</th><th className="px-2 py-3 text-right">တကယ်ရရှိငွေ</th><th className="px-2 py-3 text-right">အကြွေးတိုးဗူး</th></tr></thead><tbody>{data.daily.map((row) => <tr key={row.date} className="border-b border-slate-100"><td className="px-2 py-3 font-bold">{row.date}</td><td className="px-2 py-3 text-right">{number(row.customers)}</td><td className="px-2 py-3 text-right font-black text-cyan-700">{number(row.bottles)}</td><td className="px-2 py-3 text-right">{money(row.amount)}</td><td className="px-2 py-3 text-right text-emerald-700">{money(row.paidAmount)}</td><td className="px-2 py-3 text-right text-violet-700">{number(row.creditBottles)}</td></tr>)}</tbody></table></div></details>
          <details className="group rounded-2xl border border-cyan-200 bg-cyan-50/60 p-4 shadow-sm sm:p-5">
            <summary className="cursor-pointer list-none text-xl font-black text-cyan-950 marker:hidden"><span className="mr-2 inline-block transition-transform group-open:rotate-90">▶</span>Item အလိုက် စုစုပေါင်း</summary><div className="mt-3 overflow-x-auto"><table className="w-full min-w-[620px] text-sm"><thead><tr className="border-b-2 border-cyan-200 text-left text-xs text-cyan-800"><th className="px-2 py-3">Item</th><th className="px-2 py-3">ဆံ့/ကဒ်</th><th className="px-2 py-3 text-right">ကဒ်</th><th className="px-2 py-3 text-right">ဗူး</th><th className="px-2 py-3 text-right">ငွေ</th></tr></thead><tbody>{items.map((item) => <tr key={`${item.productKey}-${item.capacity}`} className="border-b border-cyan-100"><td className="px-2 py-3 font-bold">{item.productName}</td><td className="px-2 py-3">{number(item.capacity)}</td><td className="px-2 py-3 text-right">{number(item.cardCount)}</td><td className="px-2 py-3 text-right font-black text-cyan-700">{number(item.bottleCount)}</td><td className="px-2 py-3 text-right font-bold">{money(item.totalAmount)}</td></tr>)}</tbody></table></div></details>
          <details className="group rounded-2xl border border-sky-200 bg-sky-50/60 p-4 shadow-sm sm:p-5">
            <summary className="cursor-pointer list-none text-xl font-black text-sky-950 marker:hidden"><span className="mr-2 inline-block transition-transform group-open:rotate-90">▶</span>Tube နဲ့ သက်ဆိုင်သော ဗူးစာရင်း</summary><p className="mt-1 text-sm font-semibold text-sky-800">သတ်မှတ်ထားသော Tube တစ်မျိုးချင်းစီနဲ့ ထုတ်လုပ်သော ဗူးအမျိုးအစားများကို စုစည်းပြထားပါသည်။</p><div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">{tubeBottleGroups.length ? tubeBottleGroups.map((group) => <div key={group.label} className="rounded-xl border border-sky-100 bg-white p-4"><div className="flex items-center justify-between gap-2"><h3 className="font-black text-sky-950">{group.grams} · {group.color}</h3><span className="rounded-full bg-sky-100 px-2 py-1 text-xs font-black text-sky-800">{group.label}</span></div><ul className="mt-3 space-y-2">{group.bottles.map((bottle) => <li key={bottle.productKey} className="flex items-start justify-between gap-3 border-b border-sky-50 pb-2 text-sm"><span className="font-bold text-slate-800">{bottle.productName} <span className="font-normal text-slate-500">({number(bottle.capacity)} ဆံ့)</span></span><span className="shrink-0 text-right font-black text-sky-700">{number(bottle.bottleCount)} ဗူး</span></li>)}</ul></div>) : <p className="rounded-xl border border-dashed border-sky-200 bg-white p-6 text-center font-bold text-slate-500 md:col-span-2 xl:col-span-3">Tube mapping ထည့်ထားသော ဗူးအမျိုးအစား မရှိသေးပါ။</p>}</div></details>
          <details className="group rounded-2xl border border-violet-200 bg-violet-50/60 p-4 shadow-sm sm:p-5">
            <summary className="cursor-pointer list-none text-xl font-black text-violet-950 marker:hidden"><span className="mr-2 inline-block transition-transform group-open:rotate-90">▶</span>Customer အလိုက် စုစုပေါင်း</summary><div className="mt-3 space-y-2">{(data.customers || []).map((customer) => <div key={customer.customer.id} className="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-violet-100 bg-white p-3"><span className="font-black text-slate-900">{customer.customer.name}</span><span className="font-bold text-violet-800">{number(customer.totalBottles)} ဗူး · {money(customer.totalAmount)} · {customer.transactions} ကြိမ်</span></div>)}</div></details>
        </> : null}
      </div>
    </main>
  );
