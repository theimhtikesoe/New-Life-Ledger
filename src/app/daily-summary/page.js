        ...
        {loading ? <div className="rounded-xl bg-white p-6 text-center text-[13px] text-slate-600 shadow-sm sm:p-8 sm:text-sm">Summary ရယူနေသည်...</div> : data && (
          <>
            ...
            <section className="grid grid-cols-1 gap-3 sm:gap-5 lg:grid-cols-3">
              ...
              <div className="rounded-xl border border-slate-200 bg-white p-3.5 shadow-sm sm:p-5"><h2 className="text-base font-semibold text-slate-900 sm:text-lg">Payment Type</h2><p className="mt-1 text-xs leading-5 text-slate-500 sm:text-sm">Ledger ငွေချေမှုနဲ့ လက်ငင်းရောင်းရငွေကို သီးခြားခွဲပြထားပါတယ်။</p><div className="mt-3 space-y-3">{paymentEntries.length ? <section className="rounded-lg border border-emerald-200 bg-emerald-50/70 p-3"><p className="text-xs font-bold uppercase tracking-wide text-emerald-700 sm:text-sm">Ledger ငွေချေမှုစုစုပေါင်း · Payment Total</p><p className="mt-1 text-xs leading-5 text-emerald-800">အောက်က Ledger payment အမျိုးအစားများကိုသာ ပေါင်းထားတာဖြစ်ပြီး လက်ငင်းရောင်းရငွေ မပါဝင်ပါ။</p><div className="mt-2 space-y-1.5"><div className="flex items-center justify-between gap-3 rounded-md bg-emerald-100/80 px-3 py-2"><span className="text-sm font-bold text-emerald-800">စုစုပေါင်း</span><strong className="whitespace-nowrap text-base font-bold text-emerald-900 sm:text-lg">{formatMoney(paymentTotal)}</strong></div>{paymentEntries.map(([type, amount]) => <div key={type} className="flex items-center justify-between gap-3 rounded-md bg-white/80 px-3 py-2"><span className="text-sm font-semibold text-emerald-800 sm:text-base">{type}</span><strong className="text-base font-bold text-emerald-900 sm:text-lg">{formatMoney(amount)}</strong></div>)}</div></section> : <p className="text-[13px] text-slate-500 sm:text-sm">Ledger ငွေချေမှုမရှိသေးပါ။</p>}{cashPaymentEntries.length ? <section className="rounded-lg border border-cyan-200 bg-cyan-50/80 p-3"><div className="flex items-center justify-between gap-3"><p className="text-xs font-bold uppercase tracking-wide text-cyan-700 sm:text-sm">လက်ငင်းရောင်းရငွေ · Cash Sales</p><strong className="whitespace-nowrap text-sm font-bold text-cyan-900 sm:text-base">{formatMoney(data.summary.cashAmount)}</strong></div><p className="mt-1 text-xs leading-5 text-cyan-800">ဒီအောက်က CASH / KPAY တွေက လက်ငင်းရောင်းရငွေ စုစုပေါင်းရဲ့ ခွဲခြမ်းချက်ဖြစ်ပြီး အပေါ်က Ledger Payment Total ထဲ မပါဝင်ပါ။</p><div className="mt-2 space-y-1.5">{cashPaymentEntries.map(([type, amount]) => <div key={`cash-${type}`} className="flex items-center justify-between gap-3 rounded-md bg-white/80 px-3 py-2"><span className="text-sm font-semibold text-cyan-800 sm:text-base">{type}</span><strong className="text-base font-bold text-cyan-900 sm:text-lg">{formatMoney(amount)}</strong></div>)}</div></section> : null}{cashSaleTypeEntries.length ? <section className="rounded-lg border border-violet-200 bg-violet-50/80 p-3"><p className="text-xs font-bold uppercase tracking-wide text-violet-700 sm:text-sm">ရောင်းအမျိုးအစား · Sale Breakdown</p><div className="mt-3 space-y-2.5">{cashSaleTypeEntries.map(([type, detail]) => <div key={`cash-sale-type-${type}`} className={`flex items-center justify-between gap-3 rounded-md bg-white/80 px-3 py-2.5 ${type === "WHOLESALE" ? "text-amber-800" : "text-violet-800"}`}><span className="text-sm font-semibold sm:text-base">{cashSaleTypeLabel(type)}</span><strong className="text-base font-bold sm:text-lg">{detail.count} ခု / {formatMoney(detail.amount)}</strong></div>)}</div></section> : null}</div></div>
            </section>
          </>
        )}
      </div>
