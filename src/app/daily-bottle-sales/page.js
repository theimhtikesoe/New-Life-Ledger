    ...
    <main className="app-page-main">
      <div className="app-page-container space-y-4">
        ...
        <section className="grid gap-3 sm:grid-cols-3 lg:grid-cols-4">
          ...
          <div className="rounded-xl border border-cyan-200 bg-cyan-50 p-4"><p className="text-xs font-bold text-cyan-700">စုစုပေါင်း ရောင်းဗူး</p><p className="mt-1 text-2xl font-black text-cyan-900">{loading ? "—" : Number(data?.totalBottles || 0).toLocaleString()} ဗူး</p><p className="mt-1 text-[11px] font-bold text-cyan-700">လက်ငင်း + အကြွေးတိုး (ငွေချေဗူး မထပ်ပေါင်း)</p></div>
          <div className="rounded-xl border border-amber-200 bg-amber-50 p-4"><p className="text-xs font-bold text-amber-700">စုစုပေါင်း ရောင်းတန်ဖိုး</p><p className="mt-1 text-2xl font-black text-amber-900">{loading ? "—" : money(data?.totalAmount)}</p><p className="mt-1 text-[11px] font-bold text-amber-700">လက်ငင်း + အကြွေးတိုး (ငွေချေငွေ မထပ်ပေါင်း)</p></div>
          ...
          <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-4"><p className="text-xs font-bold text-emerald-700">တကယ်ရရှိငွေ</p><p className="mt-1 text-2xl font-black text-emerald-900">{loading ? "—" : money(data?.totalPaidAmount)}</p><p className="mt-1 text-[11px] font-bold text-emerald-700">လက်ငင်း + ငွေချေ သာ</p></div>
          <div className="rounded-xl border border-teal-200 bg-teal-50 p-4"><p className="text-xs font-bold text-teal-700">ငွေရပြီးဗူး</p><p className="mt-1 text-2xl font-black text-teal-900">{loading ? "—" : (Number(data?.paidBottleSales?.totalBottles || 0) + Number(data?.cashBottleSales?.totalBottles || 0)).toLocaleString()} ဗူး</p><p className="mt-1 text-[11px] font-bold text-teal-700">လက်ငင်း + Link ချိတ်ငွေချေ · စုစုပေါင်းထဲ မထပ်ပေါင်း</p></div>
          <div className="rounded-xl border border-violet-200 bg-violet-50 p-4"><p className="text-xs font-bold text-violet-700">အကြွေးတိုးဗူး</p><p className="mt-1 text-2xl font-black text-violet-900">{loading ? "—" : Number(data?.creditBottleSales?.totalBottles || 0).toLocaleString()} ဗူး</p><p className="mt-1 text-[11px] font-bold text-violet-700">ငွေမချေရသေးသော ရောင်းဗူး</p></div>
        </section>
        ...
        {!loading && (Number(data?.paidBottleSales?.totalBottles || 0) > 0 || Number(data?.paidBottleSales?.totalPaidAmount || 0) > 0 || data?.paidBottleSales?.customers?.length > 0) ? (
