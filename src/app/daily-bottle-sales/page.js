    ...
    <main className="app-page-main">
      <div className="app-page-container space-y-4">
        ...
        <section className="grid gap-3 sm:grid-cols-3 lg:grid-cols-6">
          ...
          <button type="button" onClick={() => setDifferenceModalOpen(true)} disabled={loading || !Number(data?.totalDifference || 0)} className="rounded-xl border border-orange-200 bg-orange-50 p-4 text-left transition hover:border-orange-400 hover:shadow-sm disabled:cursor-default disabled:opacity-100"><p className="text-xs font-bold text-orange-700">ကွာဟချက်</p><p className="mt-1 text-2xl font-black text-orange-900">{loading ? "—" : money(data?.totalDifference)}</p><p className="mt-1 text-[11px] font-bold text-orange-700">{loading ? "" : "အသေးစိတ်ကြည့်ရန် →"}</p></button>
          ...
          {data?.customers?.map((customerRow) => (
            <article key={customerRow.customer.id} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
              <div className="flex flex-col gap-2 border-b border-slate-100 pb-3 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div className="text-left sm:text-right"><p className="text-sm font-black text-cyan-700">{Number(customerRow.totalCards || 0).toLocaleString()} ကဒ် · {customerRow.totalBottles.toLocaleString()} ဗူး</p><p className="text-sm font-black text-amber-700">သတ်မှတ်ငွေ {money(customerRow.totalAmount)}</p>{customerRow.difference ? <p className="text-sm font-black text-orange-700">ကွာဟချက် {money(customerRow.difference)}</p> : null}<p className="text-sm font-black text-emerald-700">တကယ်ရှင်းငွေ {money(customerRow.totalPaidAmount)}</p></div>
              </div>
              ...
              <div className="mt-4 space-y-3">
                {(data?.customers || []).filter((customerRow) => Number(customerRow.difference || 0) !== 0).map((customerRow) => (
                  <article key={`difference-${customerRow.customer.id}`} className="rounded-xl border border-orange-200 bg-orange-50/60 p-3">
                    <div className="flex flex-wrap items-start justify-between gap-2"><div><h3 className="font-black text-slate-900">{customerRow.customer.name}</h3><p className="text-xs text-slate-500">{Number(customerRow.totalCards || 0).toLocaleString()} ကဒ် · {customerRow.totalBottles.toLocaleString()} ဗူး</p></div><p className="text-base font-black text-orange-800">ကွာဟချက် {money(customerRow.difference)}</p></div>
                    <div className="mt-3 grid grid-cols-1 gap-2 text-sm sm:grid-cols-3"><div className="rounded-lg bg-white p-2"><p className="text-xs font-bold text-slate-500">သတ်မှတ်ငွေ</p><p className="font-black text-slate-900">{money(customerRow.totalAmount)}</p></div><div className="rounded-lg bg-white p-2"><p className="text-xs font-bold text-slate-500">တကယ်ရှင်းငွေ</p><p className="font-black text-emerald-700">{money(customerRow.totalPaidAmount)}</p></div><div className="rounded-lg bg-white p-2"><p className="text-xs font-bold text-slate-500">ကွာဟချက်</p><p className="font-black text-orange-800">{money(customerRow.difference)}</p></div></div>
