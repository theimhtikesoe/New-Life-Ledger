          ...
          {data?.customers?.map((customerRow) => (
            <article key={customerRow.customer.id} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
              <div className="flex flex-col gap-2 border-b border-slate-100 pb-3 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div className="text-left sm:text-right"><p className="text-sm font-black text-cyan-700">{customerRow.totalBottles.toLocaleString()} ဗူး</p><p className="text-sm font-black text-amber-700">သတ်မှတ်ငွေ {money(customerRow.totalAmount)}</p>{customerRow.totalDiscount ? <p className="text-sm font-black text-orange-700">လျှော့ {money(customerRow.totalDiscount)}</p> : null}<p className="text-sm font-black text-emerald-700">ရှင်းငွေ {money(customerRow.totalPaidAmount)}</p></div>
              </div>
