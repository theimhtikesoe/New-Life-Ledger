    ...
    <main className="app-page-main">
      <div className="app-page-container space-y-4">
        ...
        <section className="grid gap-3 sm:grid-cols-3 lg:grid-cols-6">
          ...
          <div className="rounded-xl border border-orange-200 bg-orange-50 p-4"><p className="text-xs font-bold text-orange-700">ကွာဟချက်</p><p className="mt-1 text-2xl font-black text-orange-900">{loading ? "—" : money(data?.totalDifference)}</p></div>
          ...
          {data?.customers?.map((customerRow) => (
            <article key={customerRow.customer.id} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
              <div className="flex flex-col gap-2 border-b border-slate-100 pb-3 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div className="text-left sm:text-right"><p className="text-sm font-black text-cyan-700">{customerRow.totalBottles.toLocaleString()} ဗူး</p><p className="text-sm font-black text-amber-700">သတ်မှတ်ငွေ {money(customerRow.totalAmount)}</p>{customerRow.difference ? <p className="text-sm font-black text-orange-700">ကွာဟချက် {money(customerRow.difference)}</p> : null}<p className="text-sm font-black text-emerald-700">တကယ်ရှင်းငွေ {money(customerRow.totalPaidAmount)}</p></div>
              </div>
