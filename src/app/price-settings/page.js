        ...
        <form onSubmit={savePrices} className="space-y-4">
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {BOTTLE_GROUPS.map((category) => (
                <label key={category.key} className="rounded-xl border border-violet-100 bg-violet-50/60 p-3 text-sm font-bold text-slate-800">
                  ...
                  <div className="mt-2 flex items-center gap-2"><input type="number" min="0" step="1" inputMode="numeric" value={categoryPrices[category.key] || ""} onChange={(event) => setCategoryPrice(category.key, event.target.value)} placeholder="မသတ်မှတ်ရသေး" className="h-11 min-w-0 flex-1 rounded-lg border border-violet-200 bg-white px-3 text-right font-black" /><span className="text-xs font-black">Ks/ဗူး</span></div>
                </label>
              ))}
            </div>
          </section>
          ...
          <section className="rounded-2xl border border-cyan-200 bg-white p-4 shadow-sm sm:p-5">
            ...
            {loading ? <p className="py-10 text-center text-sm text-slate-500">Catalog နှင့် စျေးနှုန်းများ ရယူနေသည်...</p> : <div className="mt-4 space-y-2">{visibleItems.map((item) => {
              ...
              return <div key={item.productKey} className="grid gap-2 rounded-xl border border-slate-200 bg-slate-50 p-3 sm:grid-cols-[minmax(0,1fr)_120px_150px_auto] sm:items-center"><div><p className="font-black text-slate-900">{item.productName}</p><p className="text-xs text-slate-500">{item.capacity} ဗူး/ကဒ် · လက်ရှိ {effectivePrice ? `${money(effectivePrice)}/ဗူး` : "စျေးမသတ်မှတ်ရသေး"}</p></div><div className="text-xs font-semibold text-slate-600">ကဒ်တစ်ကဒ်<br /><span className="font-black text-slate-900">{effectivePrice ? money(Number(effectivePrice) * item.capacity) : "—"}</span></div><label className="text-xs font-bold text-slate-700">Item Override<input type="number" min="0" step="1" inputMode="numeric" value={itemPrice} onChange={(event) => setItemPrice(item.productKey, event.target.value)} placeholder={categoryPrice ? `${categoryPrice} (Category)` : "စျေးထည့်ပါ"} className="mt-1 h-10 w-full rounded-lg border border-cyan-200 bg-white px-2 text-right font-black" /></label><button type="button" onClick={() => clearItemPrice(item.productKey)} disabled={!itemPrice} className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs font-bold text-slate-600 disabled:opacity-40">Category သုံး</button></div>;
            })}</div>}
          </section>
