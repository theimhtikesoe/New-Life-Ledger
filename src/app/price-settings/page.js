...
function isBottleItem(itemOrKey) {
  return itemOrKey?.productType === "bottle";
}
  ...
  async function loadPrices(nextDate = date) {
    ...
    try {
      ...
      for (const item of data.catalog || []) {
        ...
        const tubeType = isBottleItem(item) ? (data.tubeMappings?.[item.productKey] || item.tubeType || "") : "";
        ...
        <form onSubmit={savePrices} className="space-y-4">
          ...
          <section className="rounded-2xl border border-cyan-200 bg-white p-4 shadow-sm sm:p-5">
            ...
            {loading ? <p className="py-10 text-center text-sm text-slate-500">Catalog နှင့် စျေးနှုန်းများ ရယူနေသည်...</p> : <div className="mt-4 min-w-0 space-y-3">{visibleItems.map((item) => {
              ...
              return <article key={item.productKey} className="min-w-0 overflow-hidden rounded-2xl border border-slate-200 bg-slate-50 p-3 shadow-sm sm:p-4">
                <div className="grid min-w-0 gap-3 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.6fr)] lg:items-start">
                  <div className="min-w-0">
                    <div className="flex min-w-0 items-start justify-between gap-2">
                      <div className="min-w-0"><p className="break-words font-black text-slate-900">{item.productName}</p><p className="mt-1 inline-flex max-w-full break-words rounded-full bg-sky-100 px-2 py-0.5 text-xs font-black text-sky-800">{isCap ? "အဖုံး သီးသန့်" : isMultiUnitPrice ? packagingDetails : isTube ? `${item.capacity} pcs / ထုပ်` : `${item.capacity} ဆံ့ / ကဒ်`}</p></div>
                      <div className="shrink-0 rounded-lg bg-amber-100 px-2 py-1 text-right text-[11px] font-semibold text-amber-900">{quantityLabel}<br /><span className="font-black">{effectivePrice ? money(unitTotal) : "—"}</span></div>
                    </div>
                    <p className="mt-2 break-words text-xs text-slate-500">လက်ရှိ {currentPriceText}</p>
                  </div>
                  <div className="grid min-w-0 gap-3 md:grid-cols-2">
                    <div className="min-w-0">{isMultiUnitPrice ? (isGlue ? <div className="grid min-w-0 grid-cols-1 gap-2 rounded-xl border border-cyan-200 bg-white p-3 text-[11px] font-bold sm:grid-cols-2"><label className="min-w-0">တစ် kg စျေး<input type="number" min="0" step="1" value={itemPrice.perKg} onChange={(event) => setPackagingPrice(item.productKey, "perKg", event.target.value)} placeholder="မထည့်ရသေး" className="mt-1 h-10 w-full min-w-0 rounded-lg border border-cyan-200 px-2 text-right font-black" /></label><label className="min-w-0">တစ်အိတ်စျေး<input type="number" min="0" step="1" value={itemPrice.perSack} onChange={(event) => setPackagingPrice(item.productKey, "perSack", event.target.value)} placeholder="မထည့်ရသေး" className="mt-1 h-10 w-full min-w-0 rounded-lg border border-cyan-200 px-2 text-right font-black" /></label></div> : <div className="grid min-w-0 grid-cols-1 gap-2 rounded-xl border border-cyan-200 bg-white p-3 text-[11px] font-bold sm:grid-cols-2"><label className="min-w-0">တစ်လုံးစျေး<input type="number" min="0" step="1" value={itemPrice.perPiece} onChange={(event) => setPackagingPrice(item.productKey, "perPiece", event.target.value)} placeholder="မထည့်ရသေး" className="mt-1 h-10 w-full min-w-0 rounded-lg border border-cyan-200 px-2 text-right font-black" /></label><label className="min-w-0">တစ်ထုပ်စျေး<input type="number" min="0" step="1" value={itemPrice.perPack} onChange={(event) => setPackagingPrice(item.productKey, "perPack", event.target.value)} placeholder="မထည့်ရသေး" className="mt-1 h-10 w-full min-w-0 rounded-lg border border-cyan-200 px-2 text-right font-black" /></label><label className="min-w-0">တစ်ပေါင်စျေး<input type="number" min="0" step="1" value={itemPrice.perLb} onChange={(event) => setPackagingPrice(item.productKey, "perLb", event.target.value)} placeholder="မထည့်ရသေး" className="mt-1 h-10 w-full min-w-0 rounded-lg border border-cyan-200 px-2 text-right font-black" /></label><label className="min-w-0">ဆာလာအိတ်စျေး<input type="number" min="0" step="1" value={itemPrice.perSack} onChange={(event) => setPackagingPrice(item.productKey, "perSack", event.target.value)} placeholder="မထည့်ရသေး" className="mt-1 h-10 w-full min-w-0 rounded-lg border border-cyan-200 px-2 text-right font-black" /></label></div>) : <label className="block text-xs font-bold text-slate-700">Item Override<input type="number" min="0" step="1" inputMode="numeric" value={itemPrice} onChange={(event) => setItemPrice(item.productKey, event.target.value)} placeholder={categoryPrice ? `${categoryPrice} (Category)` : "စျေးထည့်ပါ"} className="mt-1 h-11 w-full min-w-0 rounded-lg border border-cyan-200 bg-white px-3 text-right font-black" />{categoryPrice && !itemPrice ? <span className="mt-1 block text-[10px] font-black text-emerald-700">Category စျေး: {money(categoryPrice)}/{unitLabel}</span> : null}</label>}</div>
                    {isBottleItem(item) ? <fieldset className="min-w-0 rounded-xl border border-cyan-200 bg-white p-3"><legend className="px-1 text-xs font-bold text-slate-700">သတ်မှတ် Tube (ဗူးခွံအတွက်)</legend><div className="grid min-w-0 grid-cols-1 gap-1 sm:grid-cols-2">{TUBE_PRODUCT_TYPES.map((tubeType) => <label key={tubeType} className="flex min-w-0 items-start gap-2 text-xs font-bold"><input type="checkbox" checked={mappedTubeTypes.includes(tubeType)} onChange={(event) => toggleTubeMapping(item.productKey, tubeType, event.target.checked)} /><span className="break-words">{tubeType}</span></label>)}</div></fieldset> : null}
                  </div>
                </div>
                <div className="mt-3 flex justify-end"><button type="button" onClick={() => clearItemPrice(item.productKey)} disabled={!hasItemPrice} className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs font-bold text-slate-600 disabled:opacity-40 sm:w-auto">Category သုံး</button></div>
              </article>;
            })}</div>}
          </section>

          <div className="sticky bottom-3 z-10 flex min-w-0 justify-end"><button type="submit" disabled={saving || loading} className="w-full rounded-xl bg-amber-500 px-5 py-4 text-lg font-black text-white shadow-lg shadow-amber-900/20 hover:bg-amber-600 disabled:opacity-50 sm:w-auto">{saving ? "သိမ်းနေသည်..." : "စျေးနှုန်းသတ်မှတ်ချက် နှင့် သတ်မှတ် Tube သိမ်းမည်"}</button></div>
        </form>
      </div>
    </main>
  );
