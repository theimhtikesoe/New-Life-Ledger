...
export default function PackagingBagStockPage() {
  ...
  const movementSummary = useMemo(() => compactMovements(data?.movements || []), [data]);
  ...
  return <main className="app-page-main"><div className="app-page-container app-page-surface space-y-4 pt-5 sm:pt-6">
    ...
    <div className="grid gap-3 lg:grid-cols-2"><CompactMovementCard title={`ယနေ့ အိတ်ခွံသုံးစွဲမှု · ${date}`} rows={dailyUsed} empty="ဒီနေ့ သုံးစွဲမှု မရှိသေးပါ။" tone="rose" /><CompactMovementCard title={`ယနေ့ အသစ်ဝင် Stock · ${date}`} rows={dailyAdded} empty="ဒီနေ့ အသစ်ဝင် Stock မရှိသေးပါ။" tone="emerald" /></div>

    <details className="rounded-2xl border border-slate-200 bg-white shadow-sm"><summary className="cursor-pointer list-none px-4 py-3 font-black text-slate-900">အိတ်ခွံ Movement စုစည်းမှတ်တမ်း <span className="ml-2 text-xs font-bold text-slate-500">({movementSummary.length} စုစည်းချက်)</span></summary><div className="border-t border-slate-100 p-3"><div className="overflow-x-auto"><table className="min-w-full text-sm"><thead className="text-left text-xs font-black text-slate-600"><tr><th className="px-3 py-2">ရက်စွဲ</th><th className="px-3 py-2">အိတ်အရွယ်</th><th className="px-3 py-2">အမျိုးအစား</th><th className="px-3 py-2 text-right">အရေအတွက်</th></tr></thead><tbody className="divide-y divide-slate-100">{movementSummary.map((row) => <tr key={row.key}><td className="px-3 py-2 text-xs font-bold text-slate-500">{row.date}</td><td className="px-3 py-2 font-bold">{row.productName}</td><td className={`px-3 py-2 font-bold ${row.quantity < 0 ? "text-rose-700" : "text-emerald-700"}`}>{row.type}</td><td className={`px-3 py-2 text-right font-black ${row.quantity < 0 ? "text-rose-700" : "text-emerald-700"}`}>{row.quantity < 0 ? "−" : "+"}{formatNumber(Math.abs(row.quantity))} အိတ်</td></tr>)}</tbody></table></div></div></details>
  </div></main>;
}
