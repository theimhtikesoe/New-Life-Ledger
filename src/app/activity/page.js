...
function readActivitySnapshot(date, actor, action) {
  ...
  try {
    ...
    return { found: true, logs: Array.isArray(snapshot?.logs) ? snapshot.logs.filter((log) => !isOrderWorkflowActivity(log) && !isDailySalesActivity(log) && !isCustomerEditActivity(log) && !isProductionReportSubmitActivity(log) && !isProductionWorkerCreateActivity(log)) : [] };
  } catch {
...
async function fetchLogs(query) {
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    ...
    try {
      ...
      return Array.isArray(body.data) ? body.data.filter((log) => !isOrderWorkflowActivity(log) && !isDailySalesActivity(log) && !isCustomerEditActivity(log) && !isProductionReportSubmitActivity(log) && !isProductionWorkerCreateActivity(log)) : [];
    } catch (error) {
      lastError = error;
