...
export default function ActivityPage() {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => document.getElementById(`activity-${match.id}`)?.scrollIntoView({ behavior: "smooth", block: "center" }), 80);
      ...
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-5">
        ...
        <section id="activity-results" className="rounded-xl border border-slate-200 bg-white p-4 sm:p-5">
          ...
          {loading ? <p className="py-10 text-center text-slate-500">မှတ်တမ်းများ ရယူနေသည်...</p> : <>
            <div className="space-y-3 md:hidden">{logs.length ? logs.map((log) => { const metadata = log.metadata || {}; const isLegacy = log.eventSource === "legacy"; const isHighlighted = highlightedId === String(log.id); return <article id={`activity-${log.id}`} key={`mobile-${log.id}`} className={`rounded-xl border bg-white p-4 shadow-sm transition ${isHighlighted ? "border-amber-500 ring-2 ring-amber-300 shadow-amber-100" : "border-slate-200"}`}><div className="flex items-start justify-between gap-3"><div><p className="text-xs text-slate-500">{formatMyanmarDateTime(log.createdAt)}</p><p className="mt-1 font-medium text-slate-800">{log.actorName || ""}</p></div><span className={`rounded-full px-2 py-1 text-xs font-medium ${log.action === "PAYMENT" ? "bg-emerald-100 text-emerald-700" : log.action === "DEBT_INCREASE" ? "bg-rose-100 text-rose-700" : log.action.includes("DELETE") ? "bg-amber-100 text-amber-700" : "bg-blue-100 text-blue-700"}`}>{actionLabel(log.action)}</span></div><div className="mt-3 border-t border-slate-100 pt-3"><p className="font-medium text-slate-800">{log.entityLabel || ""}</p><p className="mt-1 text-sm text-slate-600">{log.summary}</p></div><div className="mt-3 grid grid-cols-2 gap-3 border-t border-slate-100 pt-3 text-xs"><div><p className="text-slate-500">ပမာဏ</p><p className="mt-1 font-medium text-slate-800">{money(metadata.amount) || "-"}</p></div><div><p className="text-slate-500">Payment</p><p className="mt-1 font-medium text-slate-800">{metadata.paymentType || "-"}</p></div></div><div className="mt-3 flex items-center justify-between gap-2 text-xs"><span className="text-slate-600">{metadata.note || ""}</span><span className={`rounded-full px-2 py-1 ${isLegacy ? "bg-slate-100 text-slate-600" : "bg-cyan-100 text-cyan-700"}`}>{isLegacy ? "အရင်စာရင်း" : "အသစ်မှတ်တမ်း"}</span></div></article>; }) : <div className="rounded-xl border border-slate-200 px-4 py-8 text-center text-sm text-slate-500">ဒီနေ့လုပ်ဆောင်ချက်မရှိသေးပါ။</div>}</div>
            <div className="hidden overflow-x-auto md:block"><table className="w-full min-w-[1180px] text-left text-sm"><thead className="border-b border-slate-200 text-xs uppercase text-slate-500"><tr><th className="px-3 py-3">စာရင်းနေ့/အချိန်</th><th className="px-3 py-3">လုပ်သူ</th><th className="px-3 py-3">လုပ်ဆောင်ချက်</th><th className="px-3 py-3">Customer / အကြောင်းအရာ</th><th className="px-3 py-3 text-right">ပမာဏ</th><th className="px-3 py-3">Payment</th><th className="px-3 py-3">Note</th><th className="px-3 py-3">Source</th></tr></thead><tbody className="divide-y divide-slate-100">{logs.length ? logs.map((log) => { const metadata = log.metadata || {}; const isLegacy = log.eventSource === "legacy"; const isHighlighted = highlightedId === String(log.id); return <tr id={`activity-${log.id}`} key={log.id} className={`align-top transition hover:bg-slate-50 ${isHighlighted ? "bg-amber-50 ring-2 ring-inset ring-amber-300" : ""}`}><td className="whitespace-nowrap px-3 py-3 text-xs text-slate-500">{formatMyanmarDateTime(log.createdAt)}</td><td className="px-3 py-3 font-medium text-slate-800">{log.actorName || ""}</td><td className="px-3 py-3"><span className={`rounded-full px-2 py-1 text-xs font-medium ${log.action === "PAYMENT" ? "bg-emerald-100 text-emerald-700" : log.action === "DEBT_INCREASE" ? "bg-rose-100 text-rose-700" : log.action.includes("DELETE") ? "bg-amber-100 text-amber-700" : "bg-blue-100 text-blue-700"}`}>{actionLabel(log.action)}</span></td><td className="max-w-[260px] px-3 py-3 text-slate-800"><div className="font-medium">{log.entityLabel || ""}</div><div className="mt-1 text-xs text-slate-500">{log.summary}</div></td><td className="whitespace-nowrap px-3 py-3 text-right font-medium text-slate-800">{money(metadata.amount)}</td><td className="px-3 py-3 text-slate-600">{metadata.paymentType || ""}</td><td className="max-w-[220px] px-3 py-3 text-slate-600">{metadata.note || ""}</td><td className="px-3 py-3"><span className={`rounded-full px-2 py-1 text-xs ${isLegacy ? "bg-slate-100 text-slate-600" : "bg-cyan-100 text-cyan-700"}`}>{isLegacy ? "အရင်စာရင်း" : "အသစ်မှတ်တမ်း"}</span></td></tr>; }) : <tr><td colSpan="8" className="px-3 py-10 text-center text-slate-500">ဒီနေ့လုပ်ဆောင်ချက်မရှိသေးပါ။</td></tr>}</tbody></table></div></>}
        </section>
      </div>
    </main>
  );
