...
function SharedPageHeader({ pathname, actorName }) {
  ...
  const showDashboardLink = normalizedActorName !== 'ဇွဲဇွဲ' && normalizedActorName !== 'ဖြိုးကို' && (normalizedActorName !== 'ဆောင်းဦး' || pathname === '/balance-detail' || pathname === '/debt-reconciliation' || pathname === '/prepayment-reconciliation');
  ...
  return (
    <header className="shared-page-header shared-page-header-route neon-surface neon-sweep flex min-h-[170px] flex-col justify-between rounded-2xl border border-cyan-200/80 bg-white/90 px-3 py-3 shadow-sm backdrop-blur sm:px-5 sm:py-5">
      ...
      <div className="shared-page-header-content relative flex min-h-[136px] flex-1 flex-col justify-between">
...
export default function RootLayoutClient({ children }) {
  ...
  const routeForActor = useCallback((nextActorName) => {
    ...
    if (normalizedActor === 'သက်မွန်နှင်း') return '/';
