...
import { CAP_ITEMS } from "@/lib/production-catalog";
...
export default function CapStockPage() {
  ...
  const caps = useMemo(() => {
    ...
    const catalogRows = CAP_ITEMS.filter((item) => !CAP_COLORS.includes(String(item.productName || "").replace(/^အဖုံး\s*-\s*/, ""))).map((item) => {
      ...
      return byKey.get(stockKey) || { productKey: stockKey, stockCatalogKey: item.productKey, stockType: "CAP", productName: item.productName, capacity: 5000, soldCards: 0, adjustmentCards: 0, currentCards: 0, currentBottles: 0 };
    });
