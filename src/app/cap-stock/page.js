...
export default function CapStockPage() {
  ...
  return <main className="app-page-main"><div className="app-page-container app-page-surface space-y-4 pt-5 sm:pt-6">
    <section className="rounded-2xl border border-pink-200 bg-white p-4 shadow-sm sm:p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-sm font-bold text-pink-700">အဖုံးအရောင်အလိုက် စက်ရုံလက်ကျန်</p><p className="mt-1 text-xs text-slate-500">ဗူးရောင်းတိုင်း ပုံမှန်အဖုံးနှင့် အပိုအဖုံးကို အရောင်အလိုက် အလိုအလျောက်နုတ်တွက်ထားသည်။</p></div><div className="flex flex-wrap justify-end gap-2"><div className="rounded-xl border border-pink-200 bg-pink-50 px-4 py-3 text-right"><p className="text-xs font-bold text-pink-700">စုစုပေါင်း အဖုံး Net Stock Change</p><p className="mt-1 text-xl font-black text-pink-950">{number(totalPacks)} အိတ်</p><p className="text-xs font-bold text-pink-800">စုစုပေါင်း {number(totalCaps)} ဖုံး</p></div><button type="button" onClick={() => setUsageOpen(true)} className="rounded-xl border border-rose-300 bg-rose-50 px-4 py-3 text-right shadow-sm"><p className="text-xs font-bold text-rose-700">ယနေ့ အဖုံးသုံးစွဲမှု</p><p className="mt-1 text-xl font-black text-rose-950">{number(dailyCapPieces)} ဖုံး</p><p className="text-xs font-bold text-rose-800">အသေးစိတ်ကြည့်ရန် →</p></button></div></div></section>
