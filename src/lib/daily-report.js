...
export function createReportHtml(report, fontDataUri, latinDataUri) {
  ...
  const dailySalesSummary = report.dailySalesSummary || null;
  const esc = escapeXml;
  const dailySalesSummaryBlock = dailySalesSummary
    ? `<section class="daily-sales-summary"><h2>နေ့စဉ် လက်လီ / လက်ကား ရောင်းရငွေ</h2><div class="daily-sales-grid"><div><span>လက်လီ Total</span><strong>${esc(amount(dailySalesSummary.retailTotal))}</strong></div><div><span>လက်ကား Total</span><strong>${esc(amount(dailySalesSummary.wholesaleTotal))}</strong></div><div><span>နေ့စဉ် Total</span><strong>${esc(amount(Number(dailySalesSummary.retailTotal || 0) + Number(dailySalesSummary.wholesaleTotal || 0)))}</strong></div><div><span>လက်လီ Cash</span><strong>${esc(amount(dailySalesSummary.retailCash))}</strong></div><div><span>လက်ကား Cash</span><strong>${esc(amount(dailySalesSummary.wholesaleCash))}</strong></div><div><span>Cash Total</span><strong>${esc(amount(Number(dailySalesSummary.retailCash || 0) + Number(dailySalesSummary.wholesaleCash || 0)))}</strong></div></div><div class="daily-sales-meta">စာရင်းနေ့: ${esc(dailySalesSummary.date)} · Source: ${esc(dailySalesSummary.source || "DAILY_INPUT")}${dailySalesSummary.enteredAt ? ` · ထည့်သည့်အချိန်: ${esc(formatMyanmarDate(dailySalesSummary.enteredAt))}` : ""}${dailySalesSummary.note ? `<br>မှတ်ချက်: ${esc(dailySalesSummary.note)}` : ""}</div></section>`
    : "";
