...
export function createReportHtml(report, fontDataUri, latinDataUri) {
  ...
  const customerRows = customers.map((customer) => {
    ...
    return `<tr><td>${esc(customer.customerName)}</td><td class="green">${customer.paidCount} / ${esc(amount(customer.paidAmount))}</td><td class="red">${customer.debtCount} / ${esc(amount(customer.debtAmount))}</td><td class="cash">${customer.cashCount || 0} / ${esc(amount(customer.cashAmount))}${cashTypeDetails ? `<br><span class="cash-detail">${esc(cashTypeDetails).replaceAll("&lt;br&gt;", "<br>")}</span>` : ""}</td></tr>`;
  }).join("");
  ...
  const activityRows = logs.map((log) => {
    ...
    const paymentDisplay = metadata.paymentType || (log.action === "CASH_SALE" ? "CASH" : "");
    return `<tr><td>${esc(formatMyanmarDate(log.createdAt))}</td><td>${esc(log.actorName || "")}</td><td>${esc(actionLabel(log.action))}</td><td>${esc(log.entityLabel || log.entityType || "")}</td><td>${esc(metadata.amount == null ? "" : amount(metadata.amount))}</td><td class="payment-cell">${esc(paymentDisplay)}</td><td>${esc(metadata.note || "")}</td><td>${esc(log.eventSource === "legacy" ? "အရင်စာရင်း" : "အသစ်မှတ်တမ်း")}</td></tr>`;
  }).join("");
