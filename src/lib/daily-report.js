...
export function createReportHtml(report, fontDataUri, latinDataUri) {
  ...
  const activityRows = logs.map((log) => {
    ...
    return `<tr><td>${esc(formatMyanmarDate(log.createdAt))}</td><td>${esc(log.actorName || "")}</td><td>${esc(actionLabel(log.action))}</td><td>${esc(log.entityLabel || log.entityType || "")}</td><td>${esc(metadata.amount == null ? "" : amount(metadata.amount))}</td><td class="payment-cell">${esc(paymentDisplay)}</td><td>${esc(metadata.note || "")}</td></tr>`;
  }).join("");
