...
describe("Dashboard loading recovery", () => {
  it("has a watchdog so a slow PWA/API request cannot leave the screen covered forever", () => {
    expect(source).toContain("DASHBOARD_LOADING_WATCHDOG_MS = 12000");
    expect(source).toContain("setLoadingTimedOut(true)");
    expect(source).toContain("setDataLoadError(\"Dashboard data ရယူရန် ကြာနေပါသည်။ ခဏနားပြီး ပြန်လည်ရယူပါမည်။\")");
