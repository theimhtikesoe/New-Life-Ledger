...
describe("Activity History all-action visibility", () => {
  it("requests all audit records by default without deleting or mutating them", async () => {
    ...
    expect(where.AND).not.toContainEqual({ NOT: { action: "DAILY_REPORT_SENT" } });
    expect(mocks.auditFindMany).toHaveBeenCalledTimes(1);
  });

  it("returns OrderBatch and ORDER_* records in the website activity list", async () => {
    mocks.auditFindMany.mockReset().mockResolvedValue([
    ...
    expect(response.status).toBe(200);
    expect(body.data.map((log) => log.id)).toEqual(["order", "batch", "prefixed", "payment"]);
  });
