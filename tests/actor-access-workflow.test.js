...
describe("Actor access workflow", () => {
  ...
  it("keeps Zway Zway on Production while keeping Dashboard navigation for other users", () => {
    ...
    expect(layoutSource).toContain("const zwayZwayDashboardLinkPaths = ['/production', '/production-history', '/factory-stock'];");
    expect(layoutSource).toContain("const showDashboardLink = (normalizedActorName !== 'ဇွဲဇွဲ' || zwayZwayDashboardLinkPaths.includes(pathname)) && normalizedActorName !== 'ဖြိုးကို'");
