...
describe("Myanmar report date ranges", () => {
  ...
  it("returns recent dates oldest-first including today", () => {
    expect(getRecentMyanmarDayRanges(now, 3).map((range) => range.dateLabel)).toEqual([
      "2026-08-25",
      "2026-08-26",
      "2026-08-27",
    ]);
    expect(getRecentMyanmarDayRanges(now, 99)).toHaveLength(7);
  });
